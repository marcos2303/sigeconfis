...
elseif ($accion == "getDescripcionPartidaCuenta") {
	...
	$query = mysql_query($sql) or die($sql.mysql_error());
	if (mysql_num_rows($query) != 0) $field = mysql_fetch_array($query);
	echo "|$field[cod_partida]|$field[NomPartida]|$field[CodCuenta]|$field[NomCuenta]|$field[CodCuentaPub20]|$field[NomCuentaPub20]";
}
...
elseif ($accion == "obligacion_impuestos_insertar") {
	...
	elseif ($field_impuestos['FlagImponible'] == "B") $MontoAfecto = $Afecto + $NoAfecto;
	$MontoImpuesto = $MontoAfecto * $field_impuestos['FactorPorcentaje'] / 100;
...
//	inserto linea
elseif ($accion == "caja_chica_conceptos_insertar") {
	...
	while ($field_conceptos = mysql_fetch_array($query_conceptos)) {
        ...
        <tr class="trListaBody" onclick="clk($(this), 'conceptos', 'conceptos_<?=$id?>');" id="conceptos_<?=$id?>">
            ...
            <td>
            	<input type="text" name="Fecha" style="text-align:center;" class="cell datepicker" maxlength="10" />
            </td>
			<td>
			...
			<td>
                <textarea name="Descripcion" style="height:25px;" class="cell"></textarea>
			</td>
			<td>
                <input type="text" name="MontoPagado" id="MontoPagado_<?=$id?>" value="0,00" style="text-align:right;" class="cell currency" onchange="cajaChicaMontoPagado('<?=$id?>');" onfocus="nroFocus($(this));" onblur="nroBlur($(this));" <?=$dMontoPagado?> />
			</td>
            <td>
                <select name="CodRegimenFiscal" style="width:130px;" class="cell" onChange="getOptionsSelect(this.value, 'tipo-servicio', 'CodTipoServicio_<?=$id?>', 1); caja_chica_bloquear_montos($(this).val());">
                    <?=loadSelect("ap_regimenfiscal", "CodRegimenFiscal", "Descripcion", $CodRegimenFiscal, 0)?>
                </select>
            </td>
            <td>
                <select name="CodTipoServicio" id="CodTipoServicio_<?=$id?>" style="width:130px;" class="cell">
                    <?=loadSelectDependiente("masttiposervicio", "CodTipoServicio", "Descripcion", "CodRegimenFiscal", $CodTipoServicio, $CodRegimenFiscal, 0)?>
                </select>
            </td>
			<td>
                <input type="text" name="MontoAfecto" id="MontoAfecto_<?=$id?>" value="0,00" style="text-align:right;" class="cell currency" onchange="cajaChicaMontoAfecto('<?=$id?>');" onfocus="nroFocus($(this));" onblur="nroBlur($(this));" <?=$dMontoAfecto?> />
			</td>
			<td>
                <input type="text" name="MontoNoAfecto" id="MontoNoAfecto_<?=$id?>" value="0,00" style="text-align:right;" class="cell currency" onchange="cajaChicaMontoNoAfecto('<?=$id?>');" onfocus="nroFocus($(this));" onblur="nroBlur($(this));" <?=$dMontoNoAfecto?> />
			</td>
			<td>
                <input type="text" name="MontoImpuesto" id="MontoImpuesto_<?=$id?>" value="0,00" style="text-align:right;" class="cell currency" onfocus="nroFocus($(this));" onblur="nroBlur($(this));" <?=$dMontoImpuesto?> />
			</td>
			<td>
                <input type="text" name="MontoRetencion" id="MontoRetencion_<?=$id?>" value="0,00" style="text-align:right;" class="cell currency" onfocus="nroFocus($(this));" onblur="nroBlur($(this));" <?=$dMontoRetencion?> />
			</td>
            <td width="45">
                <select name="CodTipoDocumento" class="cell">
			...
			<td width="125">
                <input type="text" name="NroDocumento" class="cell" maxlength="20" />
			</td>
			<td>
                <input type="text" name="NroRecibo" class="cell" maxlength="20" />
			</td>
			<td>
                <input type="text" name="DocFiscal" id="DocFiscal_<?=$id?>" class="cell2" maxlength="15" readonly="readonly" />
			</td>
			<td>
                <input type="hidden" name="CodProveedor" id="CodProveedor_<?=$id?>" />
                <input type="text" name="NomProveedor" id="NomProveedor_<?=$id?>" class="cell2 iEditable" readonly="readonly" onfocus="caja_chica_habilitar_proveedor('<?=$id?>');" />
                ...
                <input type="hidden" name="Distribucion" id="Distribucion_<?=$id?>" />
			</td>
		</tr>
        <?
	}
