...
extract($_POST);
extract($_GET);
include($_SERVER['DOCUMENT_ROOT']."/sigeconfis/lib/conexion.php");
connect();
$_PARAMETRO = parametros();
$Ahora = ahora();
$Ahora = substr($Ahora, 0, 10);
list($AnioActual, $MesActual, $DiaActual) = preg_split("/[\s,.-]+/", $Ahora);
...
//	FUNCION PARA CARGAR SELECTS
function loadSelect2($tabla, $campo1, $campo2, $codigo, $opt, $campos=NULL, $valores=NULL) {
	$filtro = "";
	if ($campos) {
		for($i=0;$i<count($campos);$i++) {
			$filtro .= " AND $campos[$i] = '$valores[$i]'";
	...
	switch ($opt) {
		case 0:
			$sql = "SELECT $campo1, $campo2 FROM $tabla WHERE 1 $filtro ORDER BY $campo2";
			$query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
			while (@$field = mysql_fetch_array($query)) {
				if ($field[0] == $codigo) { ?><option value="<?=$field[0]?>" selected="selected"><?=htmlentities($field[1])?></option><? }
		...
		case 1:
			$sql = "SELECT $campo1, $campo2 FROM $tabla WHERE 1 $filtro";
			$query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
			while (@$field = mysql_fetch_array($query)) {
		...
		case 0:
			$sql = "SELECT CodDetalle, Descripcion
					...
					WHERE
						Estado = 'A' AND
						CodMaestro = '".$maestro."'
					ORDER BY Descripcion,CodDetalle";
			$query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
			while ($field = mysql_fetch_array($query)) {
...
function loadSelectPeriodosNominaProcesos($CodTipoProceso, $Periodo, $CodOrganismo, $CodTipoNom, $opt) {
	switch ($opt) {
		case 1:
			...
			$query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
			while ($field = mysql_fetch_array($query)) {
				if ($field[0] == $CodTipoProceso) { ?><option value="<?=$field[0]?>" selected="selected"><?=htmlentities($field[1])?></option><? }
				else { ?><option value="<?=$field[0]?>"><?=htmlentities($field[1])?></option><? }
			}
			break;
	}
...
function loadControlNominas($CodOrganismo, $CodTipoNom) {
	...
	$query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
	while ($field = mysql_fetch_array($query)) {
		if ($field[0] == $CodTipoNom) { ?><option value="<?=$field[0]?>" selected="selected"><?=htmlentities($field[1])?></option><? }
		else { ?><option value="<?=$field[0]?>"><?=htmlentities($field[1])?></option><? }
	}
}
...
function loadSelectGeneral($tabla, $codigo, $opt) {
	switch ($tabla) {
		...
		case "CONCEPTO-TIPO":
			$c[0] = "I"; $v[0] = "Ingresos";
			$c[1] = "D"; $v[1] = "Descuentos";
			$c[2] = "A"; $v[2] = "Aportes";
			$c[3] = "P"; $v[3] = "Provisiones";
			$c[4] = "T"; $v[4] = "Totales";
			break;
	}
...
function loadSelectPeriodosBono($Periodo, $CodOrganismo, $CodTipoNom, $opt) {
	...
	while ($field = mysql_fetch_array($query)) {
		...
		if ($field[3] == $Periodo) { ?><option value="<?=$id?>" selected="selected"><?=$field[3]?></option><? }
		else { ?><option value="<?=$id?>"><?=$field[3]?></option><? }
	}
}

//	FUNCION PARA CARGAR SELECTS
function loadSelectSemanasBono($Semana, $Periodo, $opt) {
...
function formatFechaDMA($fecha) {
	...
	@list($hora, $min, $seg) = split("[:]", $_hora);*/
        @list($_fecha, $_hora) = preg_split("/[\s]+/", @$fecha);
	@list($anio, $mes, $dia) = preg_split("/[\/.-]+/", $fecha);
	@list($hora, $min, $seg) = preg_split("/[:]+/", $_hora);
...
function formatFechaAMD($fecha) {
	...
	list($hora, $min, $seg) = split("[:]", $_hora);*/
        ...
        @list($_fecha, $_hora) = preg_split("/[\s]+/", @$fecha);
	@list($dia, $mes, $anio) = preg_split("/[\/.-]+/", $_fecha);
	@list($hora, $min, $seg) = preg_split("/[:]+/", $_hora);
...
function getDiasMes($periodo) {
	list($anio, $mes) = preg_split("/[\/.-]/", $periodo);
	$anio = intval($anio);
	$mes = intval($mes);
	$dias_mes[1] = 31;
...
function getDiaSemana($fecha) {
	...
	@list($dd, $md, $ad) = preg_split( '/[\/.-]+/', @$fecha);	
	$fecha = "$ad-$md-$dd";	
	...
	if(strpos($anio, "-"))
	{
		...
		list($ad, $md, $dd) = preg_split( '/[\/.-]+/', $fecha);	
		$fecha = "$dd-$md-$ad";	
...
function setUsuario($CodPersona) {
	...
	if($field['Nombres']!='')
	{
		$nombres = preg_split('/[\\s]+/', $field['Nombres']);
		$n1 = "$nombres[0]";
		$n2 = "$nombres[1]";
		$n3 = "$nombres[2]";
		$i1 = substr($n1, 0, 1);
...
function getDiasFeriados($fdesde, $fhasta) {
	@list($dia_desde, $mes_desde, $anio_desde)=@preg_split("/[\/.-]+/", @$fdesde); $DiaDesde = "$mes_desde-$dia_desde";
	@list($dia_hasta, $mes_hasta, $anio_hasta)=@preg_split("/[\/.-]+/", @$fhasta); $DiaHasta = "$mes_hasta-$dia_hasta";
	$sql = "SELECT * 
			FROM rh_feriados 
			WHERE 
				(FlagVariable = 'S' AND
	...
	while ($field = mysql_fetch_array($query)) {
		list($mes, $dia) = preg_split('/[\/.-]/', $field['DiaFeriado']);
...
function getDiasHabiles($desde, $hasta) {
	...
	//echo $desde."-----------".$hasta;die;
	list($dd, $md, $ad) = preg_split( '/[\/.-]+/', $desde);	
	$desde = "$ad-$md-$dd";	
...
function getFechaDias($fechad, $fechah) {
	list($dd, $md, $ad) = preg_split( '/[\/.-]+/', $fechad);	$desde = "$ad-$md-$dd";
	list($dh, $mh, $ah) = preg_split( '/[\/.-]/', $fechah);	$hasta = "$ah-$mh-$dh";	
	
	if(strlen($ah)<4)
	{
	list($dd, $md, $ad) = preg_split( '/[\/.-]+/', $fechad);	$desde = "$dd-$md-$ad";
	list($dh, $mh, $ah) = preg_split( '/[\/.-]/', $fechah);	$hasta = "$dh-$mh-$ah";			
	}
	
	$sql = "SELECT DATEDIFF(STR_TO_DATE('$hasta','%Y-%m-%d'), STR_TO_DATE('$desde','%Y-%m-%d'));";	$query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
	$field = mysql_fetch_array($query);
	//echo $hasta;die;
	//echo $sql;die;
	return $field[0];
...
function printEstadoActuacion($estado) {
	...
	else  $flag = "<img src='$path/circle_red.png' width='16' height='16' />";
	return $flag;
}
...
//	obtiene 
function getFirma($CodPersona, $swNomCompleto=NULL, $swEstado=NULL) {
	global $_PARAMETRO;
	$sql = "SELECT
				mp.Apellido1,
				mp.Apellido2,
	...
	##
	return array($NomCompleto, $Cargo.' '.$Estado.$tmp, $nivel);
}
...
function getDiasAdicionalesTrimestral($Fingreso, $FechaDesde, $FechaHasta, $Fegreso) {
	...
	if ($eDia == "" || $eDia == "00") {
		...
		if ($iAnio <= 1997 && $dMes == "06") $Cantidad = 2;
		elseif ($iAnio > 1997 && ($iMes == $dMes || $iMes == $hMes) && $iDia >= $dDia && $iDia <= $hDia && ((($eMes == $dMes || $eMes == $hMes) && $eDia >= $iDia) || (($eMes > $dMes || $eMes > $hMes)))) $Cantidad = 2;
		else $Cantidad = 0;
	}
	return $Cantidad;
}
...
//	
function getTiempoAntecedente($CodPersona) {
	$sql = "SELECT
				FechaDesde,
				FechaHasta
...
function vacacionTabla($CodPersona, $AniosOrganismo=NULL, $AniosAntecedente=NULL) {
	...
	list($AnioActual, $MesActual, $DiaActual) = split("[/.-]", substr($Ahora, 0, 10));
	$FechaActual = "$AnioActual-$MesActual-$DiaActual";
	$Fingreso = getVar3("SELECT Fingreso FROM mastempleado WHERE CodPersona = '".$CodPersona."'");
	if (!$AniosOrganismo) list($AniosOrganismo) = getEdad(formatFechaDMA($Fingreso), formatFechaDMA($FechaActual));
	if (!$AniosAntecedente) list($AniosAntecedente) = getTiempoAntecedente($CodPersona);
	if ($_PARAMETRO['VACANTECEDENT'] == "S") $NroAnio = $AniosOrganismo + $AniosAntecedente;
	else $NroAnio = $AniosOrganismo;
	$DiasDisfrutes = getVar3("SELECT DiasDisfrutes FROM rh_vacaciontabla WHERE NroAnio = '".$NroAnio."'");
	$DiasAdicionales = getVar3("SELECT DiasAdicionales FROM rh_vacaciontabla WHERE NroAnio = '".$AniosOrganismo."'");
