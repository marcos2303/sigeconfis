...
elseif ($modulo == "bancos") {
	//	nuevo registro
	if ($accion == "nuevo") {
		$CodTipoPago = getCodigo("mastbancos", "CodBanco", 4);
		//	inserto
...
elseif ($modulo == "usuarios") {
	...
	//	modificar registro
	elseif ($accion == "modificar") {
		if(!isset($_clave) or $_clave == '') $_clave = $Clave;
	...
	elseif ($accion == "autorizaciones") {
		...
		//	autorizaciones
		$autorizacion = preg_split("/[;]/", $autorizaciones);
		
		if(!isset($secuencia)) $secuencia = 0;
		foreach ($autorizacion as $linea) {
			
			list($Concepto, $FlagMostrar, $FlagAgregar, $FlagModificar, $FlagEliminar) = preg_split("/[|]+/", $linea);
			list($Grupo, $CodConcepto) = preg_split("/[-]+/", $Concepto);
			$secuencia++;
			$Estado = 'A';
			if($FlagMostrar =='N' and $FlagAgregar == "N" and $FlagModificar == "N" and $FlagEliminar == "N")
			{
				$Estado = 'I';
			}
	...
	elseif ($accion == "alterna") {
		...
		$autorizacion = preg_split("/[;]/", $autorizaciones);
		foreach ($autorizacion as $linea) {
			list($Concepto, $FlagMostrar) = preg_split("/[|]/", @$linea);
			list($CodOrganismo, $CodDependencia) = preg_split("/[.]/", @$Concepto);
			//$secuencia++;
			//	inserto
