...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="titulo">Asignaci&oacute;n de Conceptos Temporales a Trabajadores </td>
...
if ($filtrar=="DEFAULT") {
	...
	$filtro_pec.="AND ((pec.TipoAplicacion='T' AND '".$fperiodo."'>=pec.PeriodoDesde AND '".$fperiodo."'<=pec.PeriodoHasta) OR (pec.TipoAplicacion='P' AND '".$fperiodo."'>=pec.PeriodoDesde)) AND pec.CodConcepto='".$codconcepto."'";
	
	if ($fmes != "") $filtro_mes = " AND (me.Fingreso LIKE '%-".$fmes."-%')";
}
...
if ($accion=="GUARDAR-TODO") {
	$sql="SELECT 
			...
			FROM 
				mastpersonas mp 
				INNER JOIN mastempleado me ON (mp.CodPersona=me.CodPersona AND me.Estado = 'A') 
				LEFT JOIN pr_empleadoconcepto pec ON (mp.CodPersona=pec.CodPersona $filtro_pec)
			WHERE 
				mp.CodPersona<>'' $filtro $filtro_mes";
	...
	while($field_conceptos=mysql_fetch_array($query_conceptos)) {
		...
		if ($_POST[$d] <= $_POST[$h]) {
			...
			if ($monto > 0) {
				...
				$sql="DELETE FROM pr_empleadoconcepto WHERE CodPersona='".$field_conceptos['CodPersona']."' AND CodConcepto='".$codconcepto."' AND ((TipoAplicacion='T' AND (('".$_POST[$d]."'>=PeriodoDesde AND '".$_POST[$d]."'>=PeriodoHasta) OR ('".$_POST[$h]."'>=PeriodoDesde AND '".$_POST[$h]."'>=PeriodoHasta))) OR (TipoAplicacion='P' AND ('".$_POST[$d]."'>=PeriodoDesde OR '".$_POST[$h]."'>=PeriodoDesde)))";
...
?>
<form id="frmentrada" name="frmentrada" action="conceptos_temporales.php" method="POST" onsubmit="return validarConceptos(this.form);">
...
<table width="1000" class="tblFiltro">
    ...
    <tr>
        ...
        <td>
        	<input type="checkbox" name="chkmes" id="chkmes" value="1" onclick="forzarCheck('chkmes');" checked="checked" />
...
<table width="1000" class="tblBotones">
    <tr>
        <td align="right">
        	<input name="btGuardar" type="button" id="btGuardar" value="Guardar Todo" onclick="guardarConceptosTemporales(this.form);" />
        </td>
    </tr>
</table>
...
<table width="1000" class="tblLista">
    ...
    if ($forganismo!="" && $ftiponom!="" && $fperiodo!="" && $codconcepto!="") {
		...
		while($field_conceptos=mysql_fetch_array($query_conceptos)) {
			...
			$cantidad=number_format($field_conceptos['Cantidad'], 2, ',', '');
			//	-------------------
			...
			<tr class="trListaBody">
				...
				<td align="center">
					<select name="<?="h:".$id?>" id="<?="h:".$id?>" style="width:70px; text-align:center;">
					  <?=getPeriodos($phasta, $ftiponom, $forganismo, 1);?>
				   </select>
				</td>
