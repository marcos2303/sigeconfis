...
while ($field=mysql_fetch_array($query)) {
	...
	if ($cod!=$field['CodEmpleado']) {
		...
		$pdf->SetWidths(array(15, 90, 20, 15, 15));
		$pdf->SetAligns(array('C', 'L', 'C', 'C', 'C'));
		$pdf->Cell(15, 5); $pdf->Row(array($field['CodEmpleado'], ($field['NomCompleto']), '', '', ''));
		$pdf->Ln(2);
	}
	//	----------------------------------
	$pdf->SetFont('Arial', '', 7);
	...
	$pdf->Cell(15, 5); $pdf->Row(array('', $field['Parentesco'], ($field['NomFamiliar']), $Fnacimiento, $edad, $Afiliado));
