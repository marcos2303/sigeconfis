...
function Cabecera($pdf, $ftiponom, $nomina, $proceso, $periodo, $periodo_fecha) {
	...
	$pdf->Cell(190, 5, ('CORRESPONDIENTE A LO ESTABLECIDO EN LA LEY DEL REGIMEN PRESTACIONAL DE EMPLEO'), 0, 1, 'L');
...
$sql = "SELECT 
			...
			ptnec.Cantidad,
			(SELECT Monto FROM pr_tiponominaempleadoconcepto WHERE CodPersona = mp.CodPersona AND CodTipoNom = '".$ftiponom."' AND Periodo = '".$fperiodo."' AND CodTipoproceso = '".$ftproceso."' AND CodConcepto = '0030') AS Aporte
		FROM
			mastpersonas mp
			INNER JOIN pr_tiponominaempleado ptne ON (mp.CodPersona = ptne.CodPersona)
			INNER JOIN pr_tiponominaempleadoconcepto ptnec ON (ptne.CodPersona = ptnec.CodPersona AND ptne.CodTipoNom = ptnec.CodTipoNom AND ptne.Periodo = ptnec.Periodo AND ptne.CodTipoproceso = ptnec.CodTipoProceso AND ptnec.CodConcepto = '0025')
		WHERE
...
while ($field = mysql_fetch_array($query)) {
	...
	$pdf->Row(array(number_format($field['Ndocumento'], 0, '', '.'), $field['Busqueda'], number_format($field['TotalIngresos'], 2, ',', '.'), number_format($field['Monto'], 2, ',', '.'), number_format($field['Aporte'], 2, ',', '.')));
