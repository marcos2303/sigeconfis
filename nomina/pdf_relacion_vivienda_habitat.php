...
function Cabecera($pdf, $ftiponom, $nomina, $proceso, $periodo, $periodo_fecha) {
	...
	$pdf->SetWidths(array(20, 70, 25, 25, 25, 25));
	$pdf->SetAligns(array('C', 'C', 'C', 'C', 'C'));
	$pdf->Row(array('CEDULA', 'APELLIDOS Y NOMBRES', 'SUELDO NORMAL', 'SUELDO INTEGRAL', 'RETENCIONES', 'APORTES'));
}
//---------------------------------------------------
...
while ($field = mysql_fetch_array($query)) {
	...
	$sql = "SELECT Monto
			FROM pr_tiponominaempleadoconcepto 
			WHERE CodPersona = '".$field['CodPersona']."' AND Periodo = '".$field['Periodo']."' AND CodConcepto = '0046'";
	...
	$sql = "SELECT Monto
			FROM pr_tiponominaempleadoconcepto 
			WHERE CodPersona = '".$field['CodPersona']."' AND Periodo = '".$field['Periodo']."' AND CodConcepto = '0047'";
	...
	//	integral
	$Integral = $field['TotalIngresos'] + ($field_alivac['Monto'] * 30) + ($field_alifin['Monto'] * 30);
	##
	$SueldoNormal += $field['TotalIngresos'];
	$SueldoIntegral += $Integral;
...
//---------------------------------------------------
$pdf->SetFont('Arial', 'B', 8);
$pdf->Row(array('', 'TOTAL', number_format($SueldoNormal, 2, ',', '.'), number_format($SueldoIntegral, 2, ',', '.'), number_format($sum_retenciones, 2, ',', '.'), number_format($sum_aportes, 2, ',', '.')));
//---------------------------------------------------
