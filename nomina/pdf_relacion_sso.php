...
require('funciones_globales_nomina.php');
connect();
//---------------------------------------------------
...
function Cabecera($pdf, $ftiponom, $nomina, $proceso, $periodo, $periodo_fecha) {
	...
	$pdf->Cell(190, 5, ('CORRESPONDIENTE A LA LEY DE REFORMA PARCIAL DEL SEGURO SOCIAL OBLIGATORIO'), 0, 1, 'L');
	...
	$pdf->Row(array('CEDULA', 'APELLIDOS Y NOMBRES', 'Nº', 'SALARIO', 'RETENCIONES', 'APORTES'));
}
//---------------------------------------------------
...
$lunes = NUMERO_LUNES_FECHA($_ARGS);
...
$sql = "SELECT 
			...
			ptnec.Cantidad,
			(SELECT Monto FROM pr_tiponominaempleadoconcepto WHERE CodPersona = mp.CodPersona AND CodTipoNom = '".$ftiponom."' AND Periodo = '".$fperiodo."' AND CodTipoproceso = '".$ftproceso."' AND CodConcepto = '0029') AS Aporte
		FROM
			mastpersonas mp
			INNER JOIN pr_tiponominaempleado ptne ON (mp.CodPersona = ptne.CodPersona)
			INNER JOIN pr_tiponominaempleadoconcepto ptnec ON (ptne.CodPersona = ptnec.CodPersona AND ptne.CodTipoNom = ptnec.CodTipoNom AND ptne.Periodo = ptnec.Periodo AND ptne.CodTipoproceso = ptnec.CodTipoProceso AND ptnec.CodConcepto = '0024')
		WHERE
