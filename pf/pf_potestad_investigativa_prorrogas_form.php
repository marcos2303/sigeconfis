...
elseif ($opcion == "modificar" || $opcion == "ver" || $opcion == "revisar" || $opcion == "aprobar" || $opcion == "anular") {
	...
	$sql = "SELECT
			...
			FROM
				pf_potestadprorroga p
				INNER JOIN pf_potestad vj ON (p.CodPotestad = vj.CodPotestad)
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=<?=@$action?>" method="POST" onsubmit="return potestad_investigativa_prorrogas(this, '<?=@$accion?>');">
...
<table width="800" class="tblForm">
	...
	<tr>
		...
		<td class="gallery clearfix">
        	<input type="text" id="CodPotestad" value="<?=@$field_mast['CodPotestad']?>" style="width:110px;" disabled="disabled" />
            <a href="../lib/listas/listado_potestad_investigativa.php?filtrar=default&cod=&nom=&ventana=prorrogas&iframe=true&width=1100&height=525" rel="prettyPhoto[iframe1]" style=" <?=$display_modificar?>">
...
<table width="1075" class="tblLista">
	...
	if ($opcion != "nuevo") {
		$sql = "SELECT
				...
				WHERE vjd.CodPotestad = '".$field_mast['CodPotestad']."'
