...
function actuacion_fiscal(form, accion) {
	...
	var error = "";
	var ObjEspe = "";
	//alert($( "input[name*='ObjEspe']" ).val());
	//return false;
		$("input[name*='ObjEspe']").each(function() {  // first pass, create name mapping
			//alert($(this).val());
			if($(this).val()!='')
			{
				ObjEspe+= $(this).val() + ";";
	...
	if(ObjEspe == "")
	{
		...
		ObjEspe = "ObjetivosEspecificos=" + ObjEspe;
	}
	post+= ObjEspe + "&";
	...
	for(var i=0; n=form.elements[i]; i++) {
		...
		//	errores
		if ((n.id == "CodOrganismo" || n.id == "CodDependencia" || n.id == "CodTipoActuacion" || n.id == "FechaInicio" || n.id == "CodProceso" || n.id == "ObjetivoGeneral" || n.id == "CodOrganismoExterno") && n.value.trim() == "") 
		{ 
			error = "Debe llenar los campos obligatorios"; 
			break; 
		}
		 else if (!valFecha(n.value) && n.id == "FechaInicio") 
		{ 
			error = "Formato de Fecha Incorrecta (dd-mm-aaaa)"; 
			break; 
		}
		else if (!valAlfaNumerico(n.value)) 
		{ 
			error = n.value + "No se permiten caractéres especiales en los campos"; 
			break; 
		}
	...
	for(var i=0; n=frm_auditores.elements[i]; i++) {
		...
		if (n.name == "CodPersona"){
			detalles_auditores += n.value + "|";
		} 
		else if (n.name == "FlagCoordinador") {
			if (n.checked)
			{
				detalles_auditores += "S;";	
			}
			else
			{
				detalles_auditores += "N;";
			}
		}
	}
...
function actuacion_fiscal_actividades_terminar(form, accion,other) {
	//	formulario
	var post = "";
	var error = "";
	var data_files = '';
	if($("#cuenta_documentos").val() == 0)
	{
		error = 'Debe insertar un documento';
	}
	...
	var frm_documentos = document.getElementById("frmentrada");
	...
	if (error != "") {
			...
			buttons: {
				"Si": function() {
					...
					$.ajax({
						type: "POST",
						url: "lib/form_ajax.php",
						data: "modulo=actuacion_fiscal_actividades_terminar&"+post+"&detalles_documentos="+detalles_documentos+"&reprogramar=true&datafiles1="+ data_files,
						async: false,
						processData: false,
						contentType: false ,
						success: function(resp) {
				...
				"No": function() {
					...
					$.ajax({
						type: "POST",
						url: "lib/form_ajax.php",
						data: "modulo=actuacion_fiscal_actividades_terminar&"+post+"&detalles_documentos="+detalles_documentos+"&reprogramar=false&datafiles2="+ data_files,
						async: false,
						processData: false,  
						contentType: false ,						
						success: function(resp) {
		...
		$.ajax({
			...
			url: "lib/form_ajax.php",
			//data: data_files
			//data: new FormData(other),
			data: "modulo=actuacion_fiscal_actividades_terminar&"+post+"&detalles_documentos="+detalles_documentos+"&reprogramar=false&data_files=" + data_files,
