...
function getMiscelaneos($detalle, $maestro, $opt) {
	...
	switch ($opt) {
		case 0:
			$sql="SELECT CodDetalle, Descripcion FROM mastmiscelaneosdet WHERE CodMaestro='$maestro' ORDER BY Descripcion, CodDetalle";
			$query=mysql_query($sql) or die ($sql.mysql_error());
			$rows=mysql_num_rows($query);
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
...
function loadSelectOrganismosExternos($control, $codigo, $opt) {
	switch ($opt) {
		case 0:
			...
			$query = mysql_query($sql) or die ($sql.mysql_error());
			while ($field = mysql_fetch_array($query)) {
				if ($field[0] == $codigo) { ?><option value="<?=$field[0]?>" selected="selected"><?php htmlentities($field[1])?></option><?php }
				else { ?><option value="<?=$field[0]?>"><?php htmlentities($field[1])?></option><?php }
			}
			break;
...
function getDependencias($dependencia, $organismo, $opt) {
	...
	switch ($opt) {
		...
		case 3:
			...
			$sql="SELECT s.CodDependencia, o.Dependencia FROM seguridad_alterna s INNER JOIN mastdependencias o ON (s.CodDependencia=o.CodDependencia) WHERE s.Usuario='".$_SESSION["USUARIO_ACTUAL"]."' AND s.CodAplicacion='".$_SESSION["APLICACION_ACTUAL"]."' AND s.FlagMostrar='S' AND o.FlagControlFiscal='S' AND s.CodOrganismo='0001' GROUP BY s.CodDependencia ORDER BY s.CodDependencia";
			$query=mysql_query($sql) or die ($sql.mysql_error());
			$rows=mysql_num_rows($query);
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
