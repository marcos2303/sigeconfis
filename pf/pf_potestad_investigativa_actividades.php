...
if (@$lista == "terminar") {
	...
	$titulo = "Listar Detalle de Valoraciones";
	$btTerminar = "display:none;";
}
//	------------------------------------
if (@$filtrar == "default") {
	$fordenar = "a.CodFase, vj.Anio, vj.Secuencia, vj.CodPotestad";
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=pf_potestad_investigativa_actividades" method="post">
	...
	<tr>
		...
		<td>
            ...
            <select name="fordenar" id="fordenar" style="width:100px;" <?=@$dordenar?>>
                <?=loadSelectValores("ORDENAR-VALORACION-DETALLE", $fordenar, 0)?>
            </select>
		</td>
	</tr>
	<tr>
...
<table width="1050" class="tblBotones">
	<tr>
		...
		<td align="right">
			<input type="button" id="btTerminar" value="Terminar" style="width:75px; <?=@$btTerminar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pf_potestad_investigativa_actividades_terminar', 'SELF', '', 'registro');" />
            
			<input type="button" id="btVer" value="Ver" style="width:75px; <?=@$btVer?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pf_potestad_investigativa_form&opcion=ver&origen=ejecucion', 'BLANK', 'height=700, width=900, left=100, top=0, resizable=no', $('#registro').val());" />
		</td>
	</tr>
</table>
...
<table width="100%" class="tblLista">
	...
	$sql = "SELECT
			...
			FROM
				...
				INNER JOIN pf_potestad vj ON (vjd.CodPotestad = vj.CodPotestad)
	...
	while ($field = mysql_fetch_array($query)) {
		...
		?>
		<tr class="trListaBody" onclick="mClk(this, 'registro');" id="<?=@$field['CodPotestad']?>.<?=@$field['CodActividad']?>">
