...
if ($opcion == "generar") {
	...
	$display_generar = "display:none;";
	##
	$Estado = "PR";
	$FechaRegistro = date("Y-m-d");
	$FechaInicio = formatFechaAMD(getFechaFinHabiles(formatFechaDMA($field_mast["FechaTerminoReal"]), 2));
	...
	$field_mast["FechaCompletada"] = "";
	$Origen = $field_mast['Origen'];
	
}
elseif ($opcion == "modificar" || $opcion == "ver" || $opcion == "revisar" || $opcion == "aprobar" || $opcion == "cerrar") {
	if ($origen == "ejecucion") {
		list($CodPotestad, $CodActividad) = split("[.]", $registro);
		$registro = $CodPotestad;
	}
	//	consulto datos generales
	$sql = "SELECT
				vj.*,
				...
				(SELECT SUM(vjd1.Duracion)
				 FROM
				 	pf_determinaciondetalle vjd1
					INNER JOIN pf_actividades a1 ON (vjd1.CodActividad = a1.CodActividad)
				 WHERE
				 	vjd1.CodDeterminacion = vj.CodDeterminacion AND
					a1.FlagNoAfectoPlan = 'S') AS DuracionNo
			FROM
				pf_determinacion vj
				INNER JOIN mastorganismos o ON (vj.CodOrganismo = o.CodOrganismo)
				INNER JOIN mastdependencias d ON (vj.CodDependencia = d.CodDependencia)
				INNER JOIN ac_mastcentrocosto cc ON (vj.CodCentroCosto = cc.CodCentroCosto)
				...
				INNER JOIN mastpersonas p1 ON (vj.PreparadoPor = p1.CodPersona)
				LEFT JOIN mastpersonas p2 ON (vj.RevisadoPor = p2.CodPersona)
				LEFT JOIN mastpersonas p3 ON (vj.AprobadoPor = p3.CodPersona)
			WHERE vj.CodDeterminacion = '".$registro."'";
	$query_mast = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
	if (mysql_num_rows($query_mast)) $field_mast = mysql_fetch_array($query_mast);
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=<?=$action?>" method="POST" onsubmit="return determinacion_responsabilidad(this, '<?=$accion?>');">
...
<table width="800" class="tblForm">
	<tr>
		<td class="tagForm">Determinaci&oacute;n:</td>
		<td colspan="3">
        	<input type="text" id="CodDeterminacion" value="<?=$field_mast['CodDeterminacion']?>" style="width:110px;" class="codigo" disabled="disabled" />
		</td>
	</tr>
	<tr>
		<td class="tagForm">Potestad:</td>
		<td>
        	<input type="text" id="CodPotestad" value="<?=$field_mast['CodPotestad']?>" style="width:110px;" class="codigo" disabled="disabled" />
		</td>
	...
	<tr>
		...
		<td>
            <select id="CodTipoActuacion" style="width:225px;" <?=$disabled_ver?>>
                <?=loadSelect("pf_tipoactuacionfiscal", "CodTipoActuacion", "Descripcion", $field_mast['CodTipoActuacion'], 1);?>
            </select>
		</td>
	...
	<tr>
		...
		<td class="gallery clearfix">
            ...
            <a href="../lib/listas/listado_entes_externos.php?filtrar=default&cod=CodOrganismoExterno&nom=NomOrganismoExterno&cod2=CodDependenciaExterna&nom2=NomDependenciaExterna&iframe=true&width=950&height=525" rel="prettyPhoto[iframe2]" style=" <?=$display_generar?>" id="btEnte">
	...
	<tr>
		...
		<td>
            <select id="Origen" style="width:300px;" <?=$disabled_ver?>>
            	<option value="">&nbsp;</option>
                <?=getMiscelaneos($field_mast['Origen'], "ORIGENACT", 1);?>
            </select>
		</td>
...
<table width="100%" class="tblLista">
	...
	if ($opcion != "generar") {
		$sql = "SELECT
					...
					pu.DescripCargo
				FROM
					pf_determinacionauditores vja
					INNER JOIN mastpersonas p ON (vja.CodPersona = p.CodPersona)
					INNER JOIN mastempleado e ON (p.CodPersona = e.CodPersona)
					INNER JOIN rh_puestos pu ON (e.CodCargo = pu.CodCargo)
				WHERE vja.CodDeterminacion = '".$registro."'";
...
<table width="1075" class="tblLista">
	...
	if ($opcion != "generar") {
		$sql = "SELECT
					vjd.*,
					a.CodFase,
					vjd.Descripcion AS NomActividad,
					a.FlagNoAfectoPlan,
					a.FlagAutoArchivo,
				...
				WHERE vjd.CodDeterminacion = '".$registro."'
...
<table width="100%" class="tblLista">
	...
	if ($opcion != "generar") {
		$sql = "SELECT
				...
				FROM
					pf_determinacionprorroga vjp
					INNER JOIN pf_actividades a ON (vjp.CodActividad = a.CodActividad)
				WHERE vjp.CodDeterminacion = '".$registro."'
				ORDER BY Secuencia";
