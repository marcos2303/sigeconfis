<?php
list($CodPotestad, $CodActividad) = preg_split("/[.]/", $registro);
$sql = "SELECT
			vj.CodPotestad,
		...
		FROM
			pf_potestad vj
			INNER JOIN pf_potestaddetalle vjd ON (vj.CodPotestad = vjd.CodPotestad)
		...
		WHERE
			vj.CodPotestad = '".$CodPotestad."' AND
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=pf_potestad_investigativa_actividades" method="POST" onsubmit="return potestad_investigativa_actividades_terminar(this);">
...
<table width="800" class="tblForm">
	<tr>
		<td class="tagForm" width="100">Potestad:</td>
		<td>
        	<input type="hidden" id="Secuencia" value="<?=@$field['Secuencia']?>" />
        	<input type="text" id="CodPotestad" value="<?=@$field['CodPotestad']?>" style="width:110px;" class="codigo" disabled="disabled" /> - 
