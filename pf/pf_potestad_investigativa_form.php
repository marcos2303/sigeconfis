...
if (@$opcion == "generar") {
	...
	##
	$accion = "generar";
	$titulo = "Generar Potestad Investigativa";
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=<?=@$action?>" method="POST" onsubmit="return potestad_investigativa(this, '<?=@$accion?>');">
...
<input type="hidden" name="CodActuacion" id="CodActuacion" value="<?=($field_mast['CodActuacion'])?>" />
...
<table width="800" class="tblForm">
	...
	<tr>
		<td class="tagForm">Val. Jur.:</td>
		<td>
        	<input type="text" id="CodValJur" value="<?=@$field_mast['CodValJur']?>" style="width:110px;" class="codigo" disabled="disabled" />
		</td>
...
<table width="100%" class="tblLista">
	...
	if (@$opcion != "generar") {
		$sql = "SELECT
				...
				WHERE vja.CodPotestad = '".$registro."'";
...
<table width="1075" class="tblLista">
	...
	if (@$opcion != "generar") {
		$sql = "SELECT
				...
				WHERE vjd.CodPotestad = '".$registro."'
