...
if(!isset($maxlimit)) $maxlimit = '500';
if(!isset($fedoreg)) $fedoreg = '';
if(!isset($fbuscar)) $fbuscar = '';
if(!isset($fedoreg)) $fedoreg = '';
if(!isset($ventana)) $ventana = '';
if(!isset($fccosto)) $fccosto = '';
if(!isset($forganismoext)) $forganismoext = '';
if(!isset($fdependenciaext)) $fdependenciaext = '';
if(!isset($fregistrod)) $fregistrod = '';
if(!isset($fregistroh)) $fregistroh = '';
if(!isset($cccosto)) $cccosto = '';
if(!isset($corganismoext)) $corganismoext = '';
if(!isset($forganismoext)) $forganismoext = '';
if(!isset($fnomorganismoext)) $fnomorganismoext = '';
if(!isset($cdependencia)) $cdependencia = '';
if(!isset($fdependencia)) $fdependencia = '';
if(!isset($fnomdependenciaext)) $fnomdependenciaext = '';
if(!isset($fanio)) $fanio = '';
if ($lista == "todos") {
	$titulo = "Agregar análisis de riesgo";
...
if ($filtrar == "default") {
	...
	if(!isset($_SESSION["MAXLIMIT"]))$_SESSION["MAXLIMIT"] = 500;
	...
	elseif ($lista == "revisar") $filtro.=" AND rd.Estado = 'PR'";
	elseif ($lista == "aprobar") $filtro.=" AND rd.Estado = 'RV'";
}
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=pf_riesgos_lista" method="post">
...
<table width="1050" class="tblBotones">
	<tr>
		...
		<td align="right">
			<input type="button" id="btAgregar" value="Agregar" style="width:75px; <?=$btNuevo?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=riesgo_nuevo', 'gehen.php?anz=pf_riesgos_form&action=nuevo&action=nuevo&opcion=nuevo', 'BLANK', '');" /> 
			<input type="button" id="btModificar" value="Modificar" style="width:75px; <?=$btModificar?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=riesgo_modificar', 'gehen.php?anz=pf_riesgos_form&action=modificar&action=modificar&opcion=modificar', 'BLANK', '');" />  
			<input type="button" id="btVer" value="Ver" style="width:75px; <?=$btVer?>" onclick="cargarOpcionValidar2(this.form, $('#registro').val(), 'accion=riesgo_ver', 'gehen.php?anz=pf_riesgos_form&action=ver&action=ver&opcion=ver', 'BLANK', '');" />
			<input type="button" id="btRevisar" value="Revisar" style="width:75px; <?=$btRevisar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pf_riesgos_form&opcion=revisar&action=revisar', 'BLANK', '', 'registro');" />
			<input type="button" id="btAprobar" value="Aprobar" style="width:75px; <?=$btAprobar?>" onclick="cargarOpcion2(this.form, 'gehen.php?anz=pf_riesgos_form&opcion=aprobar&action=aprobar', 'BLANK', '', 'registro');" />
...
<table width="100%" class="tblLista">
	<thead>
	<tr>
		<th scope="col" width="">Actuaci&oacute;n</th>
		<th scope="col">Ente</th>
		<th scope="col" width="">F.Registro</th>
		<th scope="col" width="">F.Inicio</th>
		<th scope="col" width="">F.Termino</th>
		<th scope="col" width="">F.Termino Real</th>
		<th scope="col" width="">Estado</th>
		<th scope="col" width="">Objetivo General</th>
	</tr>
    </thead>
	...
	$sql = "SELECT
			...
			FROM
				...
				INNER JOIN seguridad_alterna sa ON (af.CodDependencia = sa.CodDependencia 
                                AND sa.CodAplicacion = '".$_SESSION["APLICACION_ACTUAL"]."' 
                                AND sa.Usuario = '".$_SESSION["USUARIO_ACTUAL"]."' 
                                AND sa.FlagMostrar = 'S')
				...
				LEFT JOIN pf_dependenciasexternas de ON (af.CodDependenciaExterna = de.CodDependencia)
                                LEFT JOIN pf_riesgos_detalle rd ON rd.CodActuacion = af.CodActuacion 
			WHERE 1 $filtro";
