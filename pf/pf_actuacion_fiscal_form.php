...
<table width="800" align="center" cellpadding="0" cellspacing="0">
	<tr>
		<td>
			...
			<ul id="tab">
			<!-- CSS Tabs -->
			<?php $cant_tabs = 4; if(isset($_REQUEST['registro']) and $_REQUEST['registro']!=''){
				$cant_tabs = 5;
			}?>
			<li id="li1" onclick="currentTab('tab', this);" class="current"><a href="#" onclick="mostrarTab('tab', 1, 4);">Informaci&oacute;n General</a></li>
			<li id="li2" onclick="currentTab('tab', this);"><a href="#" onclick="mostrarTab('tab', 2, <?php echo $cant_tabs?>);">Auditores</a></li>
			<li id="li3" onclick="currentTab('tab', this);"><a href="#" onclick="mostrarTab('tab', 3, <?php echo $cant_tabs?>);">Actividades</a></li>
			<li id="li4" onclick="currentTab('tab', this);"><a href="#" onclick="mostrarTab('tab', 4, <?php echo $cant_tabs?>);">Pr&oacute;rrogas</a></li>
			<?php if(isset($_REQUEST['registro']) and $_REQUEST['registro']!=''):?>
			<!--<li id="li5" onclick="currentTab('tab', this);"><a href="#" onclick="mostrarTab('tab', 5, <?php echo $cant_tabs?>);">Documentos</a></li>-->
			<li id="li5" onclick="currentTab('tab', this);"><a href="#" onclick="mostrarTab('tab', 5, <?php echo $cant_tabs?>);">Observaciones</a></li>
			<?php endif;?>
			</ul>
			</div>
		</td>
...
<table width="800" class="tblForm">
	...
	<tr>
		<td align="right">* Órgano de Control Fiscal:</td>
		<td>
	...
	<tr>
		...
		<td>
			<input type="text" id="Duracion" autocomplete="off" value="<?=@$field_mast['Duracion']?>" style="width:25px;" class="disabled" disabled="disabled" />
		</td>
	</tr>
	<tr>
		...
		<td>
        	<input type="text" id="DuracionNo" autocomplete="off" value="<?=intval(@$field_mast['DuracionNo'])?>" style="width:25px;" class="disabled" disabled="disabled" />
		</td>
	</tr>
	<tr>
		<td align="right">* Órgano/Ente Externo:</td>
	...
	<tr>
		...
		<td>
			<input type="text" id="DuracionTotal" autocomplete="off" value="<?=intval(@$DuracionTotal)?>" style="width:25px;" class="codigo" disabled="disabled" />
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td class="tagForm">* Objetivos Específicos:</td>
		<td colspan="3" id="listaObjetivosEspecificos">
			<a onclick="addObjetivoEspecifico()" style="display:<?php echo $display;?>; cursor: pointer;">Agregar Objetivo Específico [+]</a>
			<?php $detalle_objetivos_especificos = preg_split("/[;]+/", @$field_mast['ObjetivosEspecificos']);?>
			<?php if(count($detalle_objetivos_especificos )>0):?>
				<?php $i=0;foreach($detalle_objetivos_especificos as $det): $clase = $i;?>
					<?php if($det!=""):?>
						<div class="<?php echo $clase;?>">
							<input type="text" size="50" style="width:80%;" name="ObjEspe_<?php echo $clase;?>" id="ObjEspe_<?php echo $clase;?>" value="<?php echo $det;?>" <?php echo $disabled_ver;?>/> 
							<a title="Eliminar Objetivo Específico"  onclick="deleteObjetivoEspecifico('<?php echo $clase;?>')" style="display: <?php echo $display;?>; cursor: pointer;" class="<?php echo $clase;?>">[-]</a> 
						</div>
					<?php endif;?>
				<?php $i++;endforeach;?>
			<?php endif;?>
	...
	<table width="800" class="tblBotones">
		<tr>
			<td align="right" class="gallery clearfix">
				...
				<?php if($Estado == 'AP'):?>
				<input type="button" class="btLista" value="Documentos" onclick="verificaDocumentos(this, 'actividades')" />
				<?php endif;?>
			</td>
		</tr>
	</table>
	...
	<table width="1075" class="tblLista">
		...
		if ($opcion != "nuevo") {
			$sql = "SELECT
						...
						(afd.Duracion + afd.Prorroga) AS DiasReal
			...
			while ($field_actividades = mysql_fetch_array($query_actividades)) {
				$nroactividades++;
				$dias_cierre = 0;
				if($field_actividades['FechaTerminoCierre']!='' and  $field_actividades['FechaRegistroCierre']!='' and $field_actividades['Estado']=='TE')
				{
					$dias_cierre = getDiasHabiles( $field_actividades['FechaTerminoCierre'],$field_actividades['FechaRegistroCierre']);
				...
				<tr class="trListaBody" onclick="mClk(this, 'sel_actividades');" id="<?=$field_actividades['CodActividad']?>">
					...
					<td align="center"><input type="text" name="Duracion" autocomplete="off" style="text-align:center;" value="<?=$field_actividades['Duracion']?>" class="cell" onBlur="this.className='cell';" onFocus="this.className='cellFocus';" onchange="setFechaActividades();" <?=$disabled_ver?> /></td>
					<td align="center">
					...
					<td align="center">
						<?php echo $dias_cierre;?>
						<?php //echo $field_actividades['DiasCierre']?>
					</td>
					<td>
				...
				<?php
							...
							if(!isset($fase_cierre)) $fase_cierre = '';
							if(!isset($total_cierre)) $total_cierre = '';
				if ($field_actividades['FlagNoAfectoPlan'] == "N") {
					...
					$total_cierre += $dias_cierre;
					$fase_cierre += $dias_cierre;
				}
			}
			?>
			<tr>
...
<!--<div id="tab5" style="display:none;">
	...
	<table width="800" class="tblBotones">
		<tr>
			<td align="right" class="gallery clearfix">
				<input type="button" class="btLista" value="Listar" onclick="document.getElementById('btActividades').click();" <?=@$disabled_ver?> />
			</td>
		</tr>
	</table>		
		...
		<div style="overflow:scroll; width:800px; height:400px;">
			<table width="100%" class="tblLista">
				...
				<tbody>
					<tr class="trListaBody" id="2000" onclick="mClk(this, 'sel_notificaciones');">
						<td>Notificaciones de asignación</td>
					</tr>
					<tr class="trListaBody" id='20000' onclick="mClk(this, 'sel_cedulas');">
						<td>Cédulas de Hallazgos</td>
					</tr>
					<tr class="trListaBody" id='300000' onclick="mClk(this, 'sel_informes');">
						<td>Informes</td>
					</tr>
				</tbody>
			</table
...
<script>
	...
	function addObjetivoEspecifico()
	{
		var d = new Date();
		var clase = d.getTime(); 
		$("#listaObjetivosEspecificos").append('<div class="'+ clase +'"><input type="text" size="50" style="width:80%;" name="ObjEspe_'+clase+'" id="ObjEspe_'+clase+'"> <a title="Eliminar Objetivo Específico" onclick="deleteObjetivoEspecifico('+clase+')" style="cursor: pointer;" class="'+ clase +'">[-]</a> </div>');
		//alert(1);
	}
	function deleteObjetivoEspecifico(clase)
	{
		if(!confirm("¿Está seguro(a) de eliminar el objetivo específico?"))
		{
			return false;
		}else
		{
