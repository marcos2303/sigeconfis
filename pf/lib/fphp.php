...
function getValorProbabilidad($CodActuacion, $riesgo, $componente){
    $valor = 1;
        
        $field = array();
	$sql = "SELECT Probabilidad FROM pf_riesgos_matriz WHERE CodActuacion = '".$CodActuacion."' and IdRiesgo = '$riesgo' and IdComponente = '$componente'" ;
	$query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
	if (mysql_num_rows($query) != 0) $field = mysql_fetch_array($query);
        if(isset($field['Probabilidad']) and $field['Probabilidad']!=''){
           $valor =  $field['Probabilidad'];
        }

    return $valor;
}
function getValorImpacto($CodActuacion, $riesgo, $componente){
    $valor = 1;
        
        $field = array();
	$sql = "SELECT Impacto FROM pf_riesgos_matriz WHERE CodActuacion = '".$CodActuacion."' and IdRiesgo = '$riesgo' and IdComponente = '$componente'" ;
	//echo $sql;
        $query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
	if (mysql_num_rows($query) != 0) $field = mysql_fetch_array($query);
        if(isset($field['Impacto']) and $field['Impacto']!=''){
           $valor =  $field['Impacto'];
        }

    return $valor;
}
function getValorTotal($CodActuacion, $riesgo, $componente){
    $valor = 1;
        
        $field = array();
	$sql = "SELECT Total FROM pf_riesgos_matriz WHERE CodActuacion = '".$CodActuacion."' and IdRiesgo = '$riesgo' and IdComponente = '$componente'" ;
	//echo $sql;
        $query = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
	if (mysql_num_rows($query) != 0) $field = mysql_fetch_array($query);
        if(isset($field['Total']) and $field['Total']!=''){
           $valor =  $field['Total'];
        }
