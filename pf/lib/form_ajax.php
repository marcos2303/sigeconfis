...
if ($modulo == "riesgos") {
    ...
    //echo $promedio_total;die;
     if($accion == "nuevo"){
         
         $sql = "INSERT INTO siaceda.pf_riesgos_detalle 
	(CodActuacion, 
	TotalGeneral, 
	...
	'".$_SESSION["USUARIO_ACTUAL"]."'
        ...
        //insercion de la matriz
        foreach($probabilidad as $id_riesgo => $array_valores){

                //echo $id."<br>";
                foreach($array_valores as $componente => $valor_probabilidad){
                    $valor_impacto = $impacto[$id_riesgo][$componente];
                    $valor_total = $total[$id_riesgo][$componente];
                    
                    $insert = "INSERT INTO siaceda.pf_riesgos_matriz 
                        (IdRiesgo, 
                        IdComponente, 
                        CodActuacion, 
                        Probabilidad, 
                ...
                ";
                $query_insert = mysql_query($insert) or die(getErrorSql(mysql_errno(), mysql_error(), $insert));    

                }

                //print_r($probabilidad[$id])."<br>";
        ...
        die;
         ...
         $sql = "UPDATE siaceda.pf_riesgos_detalle set "
                 . "TotalGeneral = '$promedio_total', "
                 . "ResultadosEvaluacion = '$promedio_total' , "
        ...
        $sql = "DELETE FROM pf_riesgos_matriz WHERE CodActuacion = '".$CodActuacion."'";
 ...
 }else if ($modulo == "organismos_externos") {
	//	nuevo registro
	if ($accion== "nuevo") {		
		//	genero correlativo
...
elseif ($modulo == "actuacion_fiscal") {
	...
	if ($accion== "nuevo") {
		...
		$detalle_auditores =  preg_split("/[;]+/", @$detalles_auditores);
				
				
		foreach ($detalle_auditores as $linea) {
			//list($_CodPersona, $_FlagCoordinador) = preg_split("[|]", $linea);
                        @list($_CodPersona, $_FlagCoordinador) = preg_split("/[|]+/", $linea);
			//	inserto
		...
		//$detalle_actividades = split(";", $detalles_actividades);
                ...
                $detalle_actividades = preg_split("/[;]+/", $detalles_actividades);

		foreach ($detalle_actividades as $linea) {
			@list($_CodActividad, $_Duracion, $_FechaInicio, $_FechaTermino) = preg_split("/[|]+/", $linea);
			$_Secuencia++;
			...
			//	inserto
                        if(isset($_CodActividad) and $_CodActividad !=''){
	...
	elseif ($accion== "replanificar") {
		...
		//no eliminar las que están ejecutadas
		//
		//	actividades
		$sql = "DELETE FROM pf_actuacionfiscaldetalle WHERE CodActuacion = '".$CodActuacion."'  AND Estado = 'PR'";
		$query_delete = mysql_query($sql) or die($sql.mysql_error());
		##
		$_Secuencia = 0;
		$detalle_actividades = preg_split("/[;]+/", $detalles_actividades);
		foreach ($detalle_actividades as $linea) {
			...
			//	inserto
                        if(isset($_CodActividad) and $_CodActividad!=''){
								...
								$sql = "SELECT CodActividad FROM pf_actuacionfiscaldetalle WHERE CodActuacion = '".$CodActuacion."'  AND Estado <> 'PR' AND CodActividad = '$_CodActividad'";
								$query_select = mysql_query($sql) or die($sql.mysql_error());
								$field = mysql_fetch_array($query_select);
								if($field == false){//no está en la tabla
									$sql = "INSERT INTO pf_actuacionfiscaldetalle (
																		 CodActuacion,
																		 Secuencia,
																		 CodOrganismo,
...
elseif ($modulo == "actuacion_fiscal_actividades_terminar") {
	...
	$reprogramar = @$values['reprogramar'];
	$Duracion = @$values['Duracion'];
        $Anio = @$values['Anio'];
	$DiasCierre = @$values['DiasCierre'];
	$DiasAdelanto = @$values['DiasAdelanto'];
	$Observaciones= @$values['Observaciones'];
	$CodActuacion = @$values['CodActuacion'];
	$FechaTerminoCierre = @$values['FechaTerminoCierre'];
	$Secuencia = @$values['Secuencia'];
	$detalles_documentos = '';	
	$values['reprogramar'] = false;
	$error = '';
	$registro = $CodActuacion.".".$CodActividad;
        $carpeta = "../../archivos/pf/".$Anio."/";
	...
	if(count($values['cuenta_documentos']) == 0)
	{
		$error = 'Debe seleccionar uno o varios documentos';
		returnError($error, $registro);
	}
	if(isset($values['cuenta_documentos']) and $values['cuenta_documentos']>0)
	{	
		$i = 0;
			for($i = 0; $i < $values['cuenta_documentos'];$i++)
			{
				if($values['files']['name'][$i] == '')
				{
					$error = 'Debe seleccionar un archivo';
					returnError($error, $registro);					
				}
				
				/*if($values['Documento'][$i] == '')
				{
					$error = 'Debe indicar el nombre con que desea guardar los documentos';
					returnError($error, $registro);
				}*/
				if($values['NroDocumento'][$i] == '')
				{
					$error = 'Debe indicar el número de los documentos';
					returnError($error, $registro);
				}				
			}
	...
	if(@$values['Duracion'] == @$values['DiasCierre'] and @$values['FlagNoAfectoPlan'] == 'N')
	{
		$values['reprogramar'] = true;
	}
	...
	if(isset($_FILES))
	{
        ...
        $carpeta_actuacion = $carpeta."$CodActuacion/";
        
        if (!file_exists($carpeta_actuacion)) {
            mkdir("$carpeta_actuacion", 0777);
            //echo "The directory $carpeta was successfully created.";
        } 
               
		$detalles_documentos = '';
		//print_r($values['Documento']);die;
		$i = 0;
		foreach($values['NroDocumento'] as $documento):
			$file =  $_FILES['files']['name'][$i];
			$_NroDocumento = $values['NroDocumento'][$i];
			$_CodActividad = $values['CodActividad'];
			$_Fecha = $values['Fecha'][$i];
			$Linea = $i + 1;
			$_DocName =  $CodActuacion."_".$_CodActividad."_".$Secuencia."_".$Linea.".".pathinfo($_FILES['files']['name'][$i],PATHINFO_EXTENSION);
                        
                        if (!move_uploaded_file($_FILES['files']['tmp_name'][$i], $carpeta_actuacion.$_DocName))
                        {
                          
                            echo "Se ha presentado un inconveniente subiendo los archivos ".$_FILES['files']['name'][$i];
                            die;
                        }
			...
			$query_insert = mysql_query($sql) or die($sql.mysql_error());
		$i++;
		//$docid_mongo = saveFileMongoDb($file,'pf_actividad',$values['CodActuacion'],1,array('CodActuacion' => $values['CodActuacion'],'CodActividad' => $values['CodActividad'],'Fase' => $Secuencia,'NroDocumento' => $_NroDocumento,'Documento' => $_Documento,'Linea' => $Linea,"DocName" => $_DocName));
		endforeach;
	...
	$sql = "UPDATE pf_actuacionfiscaldetalle
			...
			WHERE
				...
				Secuencia = '".$Secuencia."'";
	$query_update = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
	//inserto en documentos de la actuacion
	...
	header('Location:../gehen.php?anz=pf_actuacion_fiscal_actividades&lista=terminar&filtrar=default&concepto=01-0005');
}
...
function returnError($error,$registro)
{
	header('Location:../gehen.php?anz=pf_actuacion_fiscal_actividades_terminar&registro='.$registro."&error=".$error);
	die;
}
?>