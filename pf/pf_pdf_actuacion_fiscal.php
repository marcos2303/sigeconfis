...
class PDF extends FPDF {
	...
	function Footer() {
		...
		$this->SetFont('Arial', 'B', 9);
		$this->SetXY(10, 240); $this->Cell(65, 5, 'CONTRALOR', 0, 0, 'L'); 
		$this->SetXY(75, 240); $this->Cell(65, 5, 'ADMINISTRADOR', 0, 0, 'L'); 
...
$sql = "SELECT
		...
		WHERE afa.CodActuacion = '".$field_mast['CodActuacion']."'
...
while ($field_actividades = mysql_fetch_array($query_actividades)) {	$i++;
	...
	$y = $pdf->GetY();
        ...
        if(isset($_REQUEST['concepto']) and $_REQUEST['concepto'] == '04-0002')//ejecución
        {
 	$pdf->Row(array('',
        ...
        if(isset($_REQUEST['concepto']) and $_REQUEST['concepto'] == '04-0001')//planificación
        {
 	$pdf->Row(array('',
					$field_actividades['Descripcion'],
					$dias,
