...
if ($opcion == "generar") {
	...
	$sql = "SELECT
			...
			FROM
				...
				LEFT JOIN mastpersonas p1 ON (af.PreparadoPor = p1.CodPersona)
	...
	if (formatFechaDMA(@$field_mast["FechaTerminoReal"]) == "") @$field_mast["FechaTerminoReal"] = date("Y-m-d");
...
elseif (@$opcion == "modificar" || $opcion == "ver" || $opcion == "revisar" || $opcion == "aprobar" || $opcion == "cerrar") {
	if (@$origen == "ejecucion") {
		list($CodValJur, $CodActividad) = preg_split("/[.]/", $registro);
		$registro = $CodValJur;
	}
	//	consulto datos generales
	$sql = "SELECT
			...
			FROM
				...
				LEFT JOIN ac_mastcentrocosto cc ON (vj.CodCentroCosto = cc.CodCentroCosto)
...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=<?=@$action?>" method="POST" onsubmit="return valoracion_juridica(this, '<?=$accion?>');">
...
<table width="100%" class="tblLista">
	...
	if ($opcion != "generar") {
		$sql = "SELECT
					...
					pu.DescripCargo
				FROM
					pf_valoracionjuridicaauditores vja
				...
				WHERE vja.CodValJur = '".$registro."'";
		//echo $sql."aaaaaqui".$registro;die;
