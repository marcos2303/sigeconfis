...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=<?=$action?>" method="POST" onsubmit="return valoracion_juridica_prorrogas(this, '<?=$accion?>');">
...
<table width="800" class="tblForm">
	...
	<tr>
		<td class="tagForm">* Valoraci&oacute;n:</td>
		<td class="gallery clearfix">
        	<input type="text" id="CodValJur" value="<?=@$field_mast['CodValJur']?>" style="width:110px;" disabled="disabled" />
            <a href="../lib/listas/listado_valoracion_juridica.php?filtrar=default&cod=&nom=&ventana=prorrogas&iframe=true&width=1100&height=525" rel="prettyPhoto[iframe1]" style=" <?=@$display_modificar?>">
		...
		$sql = "SELECT
					...
					(SELECT SUM(Prorroga)
					 ...
					 WHERE
						...
						CodActividad <> (SELECT CodActividad
										 FROM pf_valoracionjuridicadetalle
										 WHERE
											Estado = 'EJ' AND
											FlagNoAfectoPlan = 'N'
										)
				...
				WHERE vjd.CodValJur = '".$field_mast['CodValJur']."'
