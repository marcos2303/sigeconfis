...
function vouchers_abrir(registro, pagina, imprimir) {
	var url = "gehen.php?anz="+pagina+"&registro="+registro+"&iframe=true&width=1050&height=575";
...
function caja_chica_totales() {
	...
	var frm_conceptos = document.getElementById("frm_conceptos");
	for(var i=0; n=frm_conceptos.elements[i]; i++) {
		if (n.name == "MontoPagado") MontoPagado += setNumero(n.value);
		else if (n.name == "MontoAfecto") MontoAfecto += setNumero(n.value);
		else if (n.name == "MontoNoAfecto") MontoNoAfecto += setNumero(n.value);
		else if (n.name == "MontoImpuesto") MontoImpuesto += setNumero(n.value);
		else if (n.name == "MontoRetencion") MontoRetencion += setNumero(n.value);
	}
	var MontoBruto = MontoAfecto + MontoNoAfecto;
