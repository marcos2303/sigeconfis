...
$sql = "SELECT
		...
		FROM
			...
			INNER JOIN ap_retenciones r ON (op.CodOrganismo = r.CodOrganismo AND op.NroOrden = r.NroOrden AND r.Anio = op.Anio)
...
if (mysql_num_rows($query_iva) != 0) {
	...
	$pdf->AddPage();
	$pdf->Image($_PARAMETRO["PATHLOGO"].'contraloria.jpg', 8, 5, 12, 12);
	...
	$pdf->SetWidths(array(20, 27, 27, 20, 20, 19, 21, 20, 20));
	...
	$pdf->Row(array(formatFechaDMA($field_iva['FechaFactura']),
					...
					number_format($field_iva['MontoNoAfecto'], 2, ',', '.'),
...
if (mysql_num_rows($query_islr) != 0) {
	...
	$pdf->Cell(35, 5, number_format($field_islr['MontoAfecto']+$field_islr['MontoNoAfecto'], 2, ',', '.'), 0, 1, 'R');
	$pdf->Ln(3);
