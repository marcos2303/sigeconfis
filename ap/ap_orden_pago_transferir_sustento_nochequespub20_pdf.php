...
$sql = "SELECT
		...
		FROM
			...
			LEFT JOIN ac_mastplancuenta20 pc1 ON (o.CodCuentaPub20 = pc1.CodCuenta)
...
function chequeVenezuela($pdf) {
	...
	$pdf->Cell(60, 5, $field['Banco'], 0, 0, 'L');
	$pdf->SetFont('Arial', 'B', 8);
	...
	$pdf->SetFont('Arial', '', 8);
	$pdf->Cell(160, 5, $field['NomProveedorPagar'], 0, 0, 'L');
	$pdf->Ln(5);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(17, 5, 'La suma de ', 0, 0, 'L');
	$pdf->SetFont('Arial', '', 8);
	$pdf->Cell(160, 5, $monto_letras, 0, 0, 'L');
	$pdf->Ln(5);
	$pdf->Ln(5);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(15, 5, 'Proceso ', 0, 0, 'L');
	$pdf->Cell(25, 5, $field['NroProceso'], 0, 0, 'R');
	$pdf->Cell(5, 5, $field['Secuencia'], 0, 0, 'L');
	$pdf->Ln(5);
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(15, 5, 'Cta. Cte. ', 0, 0, 'L');
	$pdf->Cell(25, 5, $field['NroCuenta'], 0, 0, 'R');
	$pdf->Ln(10);
}
//---------------------------------------------------
...
class PDF extends FPDF {
	...
	function Footer() {
		...
		$this->SetXY(58.75, 233); $this->MultiCell(48.75, 3, utf8_decode($_CONFORMADO['Nombre']), 0, 'L');
		$this->SetXY(107.5, 233); $this->MultiCell(48.75, 3, utf8_decode($_REVISADO['Nombre']), 0, 'L');
		...
		$this->SetXY(58.75, 240); $this->MultiCell(48.75, 3, utf8_decode($_CONFORMADO['Cargo']), 0, 'C');
		$this->SetXY(107.5, 240); $this->MultiCell(48.75, 3, utf8_decode($_REVISADO['Cargo']), 0, 'C');
