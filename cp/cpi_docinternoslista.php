...
echo "
<form name='frmentrada' action='cpi_docinternoslista.php?limit=0&concepto=$concepto' method='POST'>
...
<input name="btEliminar" type="button" class="btLista" id="btAnular" value="Anular" onclick="AnularRegistroInterno(this.form, 'cpi_docinternoslista.php?limit=0', '1', 'PERSONAS');"/><input name="btModifRest" type="button" class="btLista" id="btModifRest" value="Modif.Rest" onclick="cargarOpcionModifRest(this.form, 'cpi_docinternoeditormodifres.php?fEstado=<?=$fEstado?>', 'SELF')"/>|
...
if ($registros!=0) {
	for ($i=0; $i<$rows; $i++) {
		...
		if($field['FechaDocumento']!='0000-00-00'){ 
		    list($a, $m, $d)=preg_split( "/[.-]+/", $field['FechaDocumento']); $f_Documento=$d.'-'.$m.'-'.$a;
		}else{ $f_Documento='';}
		...
		<tr class='trListaBody' onclick='mClk(this, \"registro\");' id='$id'>
			...
			<td align='left'>".htmlentities($field['Asunto'])."</td>
...
<script type="text/javascript" language="javascript">
	totalRegistrosDocumentoInternoListas(<?=$registros?>, "<?=$_ADMIN?>", "<?=$_INSERT?>", "<?=$_UPDATE?>", "<?=$_DELETE?>");
</script>
