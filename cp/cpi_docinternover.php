...
<?
 list($cod_tipodocumento, $cod_documentocompleto)= preg_split( "/[|]+/",@$_GET['registro']);
...
<tr>
 ...
 <? list($a, $m, $d)=preg_split( "/[.-]+/", $f['FechaRegistro']); $f_Registro=$d.'-'.$m.'-'.$a; ?>
    ...
    <td colspan="2"><select id="organismo" name="organismo" class="selectBig" onchange="getOrganismoInterno(this.id, 'dep_interna');" disabled>
      ...
      *</td><? 
	          if($f['FlagEncargaduriaEspecial']=='S'){
				  $est = "checked onclick='this.checked=true'";
	        ...
	        ?>
      <td><input type="checkbox" id="encarg_especial" name="encarg_especial" <?=$est;?> /> Encargadur&iacute;a Especial</td>
  </tr>
  <tr>
    <td class="tagForm" colspan="">Dependencia:</td>
    <td colspan="3"><select id="dep_interna" name="dep_interna" class="selectBig" disabled>
      <option value=""></option>
       <?php getDependencias($f['Cod_Dependencia'], $_SESSION['ORGANISMO_ACTUAL'], 0); ?>
    </select>
      *</td>
  </tr>
  <tr>
