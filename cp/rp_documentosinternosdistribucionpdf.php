...
function Header(){
	...
	$this->Cell(250, 5, utf8_decode('Reporte Distribución Documentos'), 0, 1, 'C');
}
//Page footer
function Footer(){
...
for($i=0; $i<$rcon; $i++){
  ...
  /// -------------------------------------------------
    @list($A,$M,$D)=preg_split('/[-]/',$fcon['FechaRegistro']);$fechaRegistro = $D.'-'.$M.'-'.$A;
  ...
  if(@$mostar==0){	 
	...
	$pdf->Cell(25, 5, utf8_decode('Dependencia:'), 0, 0, 'L'); $pdf->Cell(115, 5,$fcon['Dependencia'], 0, 1, 'L');
	$pdf->Cell(25, 5, utf8_decode('Remitente:'), 0, 0, 'L'); $pdf->Cell(115, 5,$fcon['NomCompleto'], 0, 1, 'L');
	$pdf->Cell(25, 5, utf8_decode('Cargo:'), 0, 0, 'L');$pdf->Cell(200, 5,$fcon['DescripCargo'], 0, 1, 'L');
	/*$pdf->Cell(25, 5, utf8_decode('Asunto:'), 0, 0, 'L'); $pdf->Cell(200, 5,utf8_decode($fcon['Asunto']), 0, 1, 'L');
  ...
  /// -------------------------------------------------
   ...
   $pdf->SetDrawColor(0, 0, 0); $pdf->SetFillColor(200, 200, 200); $pdf->SetTextColor(0, 0, 0);
		...
		$pdf->Cell(60, 5,utf8_decode('ASUNTO'), 1, 0, 'C', 1);
  ...
  $scon2 = "select 
		    ...
		    where 
				 ...
				 cpd.Cod_TipoDocumento = '".$fcon['Cod_TipoDocumento']."' $filtro2";
  ...
  for($j=0; $j<$rcon2; $j++){
     ...
     /// ---------------------------------------------
		 ...
		 else if($fcon2['Estado']=='PR') $estado='Preparaci&oacute;n'; 
		 else if($fcon2['Estado']=='EV') $estado='Enviado'; 
	 /// ---------------------------------------------
		  $contador = $contador + 1;
		  list($ano,$mes,$dia)=preg_split('/[-]/', $fcon2['FechaDistribucion']);	
		  $fechaDistribucion = $dia.'-'.$mes.'-'.$ano;
		  $pdf->SetDrawColor(0, 0, 0); $pdf->SetFillColor(255, 255, 255); $pdf->SetTextColor(0, 0, 0);
		  $pdf->SetFont('Arial', '', 6);
		  $pdf->SetWidths(array(19,7,38,50,50,60,18,13));
		  $pdf->SetAligns(array('C','C','C','L','L','L','C','C'));
		  //$pdf->Cell(5,5); 
		  $pdf->Row(array($fcon2['Cod_Documento'],$fcon2['Periodo'],$fcon2['NomCompleto'],$fcon2['Dependencia'],$fcon2['DescripCargo'],$fcon['Asunto'],$fechaDistribucion,$estado));
  }
}
  /// --------------------------------------------------------------
  $pdf->SetFont('Arial', '', 6);
