...
for($i=0; $i<$rcon; $i++){
  $fcon = mysql_fetch_array($qcon);
  	  /// ---------------------------------------------
		 ...
		 else if($fcon['Estado']=='PR') $estado='Preparaci&oacute;n'; 
	 ...
	 /// ---------------------------------------------
		  ...
		  //$pdf->Cell(5,5); 
		  $pdf->Row(array($fcon['Cod_DocumentoCompleto'],$fcon['Periodo'],$fcon['Descripcion'],$fcon['Dependencia'],$fcon['Asunto'],$fcon['Descp'],$fechaRegistro,$fcon['PlazoAtencion'],$estado));
  /// --------------------------------------------------------------
}
  /// --------------------------------------------------------------
  $pdf->SetFont('Arial', '', 6);
