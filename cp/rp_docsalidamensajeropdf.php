...
function Header(){
	...
	$this->SetXY(20, 15); $this->Cell(200, 5, utf8_decode('División de Correspondencia y Mensajería'), 0, 0, 'L');
	...
	$this->SetDrawColor(0, 0, 0); $this->SetFillColor(200, 200, 200); $this->SetTextColor(0, 0, 0);
		...
		$this->Cell(20, 5, 'ESTADO', 1, 1, 'C', 1); $this->Ln(5);
}
//Page footer
function Footer(){
...
if($rcon!=0){
  for($i=0;$i<$rcon;$i++){
     ...
     // -------------------------------------------------------------------------------------
     @list($A,$M,$D)=preg_split('/[-]/',$fcon['FechaRegistro']);$fechaIngreso = $D.'-'.$M.'-'.$A;
     @list($C,$E,$F)=preg_split('/[-]/',$fcon['FechaDocumento']);$fechaDocumento = $F.'-'.$E.'-'.$C;
	...
	$scon2 = "select 
				 ...
				 d.Descripcion
			 from 
				 cp_documentodistribucionext a 
				 inner join mastpersonas b on (b.CodPersona = a.Cod_PersonaResp)
				 inner join rh_puestos c on (c.CodCargo = a.Cod_CargoResp) 
				 inner join cp_tipocorrespondencia d on (d.Cod_TipoDocumento = a.Cod_TipoDocumento)
			where 
				 a.Cod_Documento = '".$fcon['Cod_Documento']."' and 
				 a.Cod_TipoDocumento = '".$fcon['Cod_TipoDocumento']."' and
				 a.CodOrganismo = '".$fcon['CodOrganismo']."' and 
				 a.Periodo = '".$fcon['Periodo']."'"; //echo $scon2;
	...
	if($rcon2!=0){
	 ...
	 // ---------------------------------------------------------
	    $pdf->SetFont('Arial','', 9);
	    $pdf->Cell(35, 5, utf8_decode('Nro.Documento: '), 0, 0,'R');
		      $pdf->Cell(15, 5, $fcon2['Cod_Documento'],0,0,'L');
			  $pdf->Cell(25, 5, 'Tipo Documento: ',0,0,'L');
			  $pdf->Cell(32, 5, $fcon2['Descripcion'],0,0,'L'); 
			  $pdf->Cell(13, 5, 'Periodo:',0,0,'L');
			  $pdf->Cell(15, 5, $fcon2['Periodo'],0,1,'L');
		
		$pdf->Cell(35, 5, utf8_decode('Responsable de Envio: '), 0, 0,'L');$pdf->Cell(20, 5, $fcon2['NomCompleto'],0,1,'L');
		...
		for($a=0; $a<$rcon3; $a++){
		  ...
		  @list($ano,$mes,$dia)=preg_split('/[-]/',$fcon['FechaRegistro']);	
