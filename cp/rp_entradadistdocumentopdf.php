...
@list($A,$M,$D)=preg_split('/[-]/',$field['FechaRegistro']);$fechaIngreso = $D.'-'.$M.'-'.$A;
@list($C,$E,$F)=preg_split('/[-]/',$field['FechaDocumentoExt']);$fechaDocumento = $F.'-'.$E.'-'.$C;
 ...
 //if($field['FlagEsParticular']=='N'){
	...
	if($r_consulta!=0){
		...
		for($a=0; $a<$r_consulta; $a++){
			...
			@list($ano,$mes,$dia)=preg_split('/[-]/',$f_consulta['FechaEnvio']);	
