...
<table width="800" class="tblLista">
    ...
    <?php
	$sql="SELECT d.CodDependencia, d.Dependencia, d.CodOrganismo, o.Organismo, s.FlagMostrar FROM mastdependencias d INNER JOIN mastorganismos o ON (d.CodOrganismo=o.CodOrganismo) LEFT JOIN seguridad_alterna s ON (d.CodOrganismo=s.CodOrganismo AND d.CodDependencia=s.CodDependencia AND s.Usuario='$registro' AND s.CodAplicacion='".$_SESSION["APLICACION_ACTUAL"]."') WHERE d.estado = 'A' ORDER BY d.CodOrganismo, d.CodDependencia";
	$query=mysql_query($sql) or die ($sql.mysql_error());
