...
if (@$fdesde != "" and @$fhasta != "") { // FECHA DE REGISTRO DEL DOCUMENTO
  ...
  @list($d, $m, $a)=preg_split('/[.-]/', @$_POST['fhasta']); $fechahasta=$a.'-'.$m.'-'.$d;
	...
	@list($a, $m, $d)=preg_split('/[.-]/', $fechadesde); $fechadesde=$d.'-'.$m.'-'.$a;
    @list($a, $m, $d)=preg_split('/[.-]/', $fechahasta); $fechahasta=$d.'-'.$m.'-'.$a;
...
if(!isset($dRemitente) or $dRemitente == '') $dRemitente = '';
if(!isset($cNroDocumento) or $cNroDocumento == '') $cNroDocumento = '';
if(!isset($NroDocumento) or $NroDocumento == '') $NroDocumento = '';
if(!isset($cDestinatario) or $dRemitente == '') $dRemitente = '';
if(!isset($dRemitente) or $dRemitente == '') $dRemitente = '';
...
<input type="button" id="btAtender" name="btAtender" class="btLista" value="Documento" onclick="cargarOpcion(this.form,'cpi_docinternodocumento.php', 'BLANK', 'height=800, width=800, left=200, top=200, resizable=yes');"/>
		</td>
	</tr>
</table>
...
if ($registros!=0) {
	for ($i=0; $i<$rows; $i++) {
		...
		@list($a, $m, $d)=preg_split( '/[.-]/', $fa['FechaRegistro']); $f_registro=$d.'-'.$m.'-'.$a;
		...
		<tr class='trListaBody' onclick='mClk(this, \"registro\");' id='$id'>
			...
			<td align='left'>".@$fdep['dependencia']."</td>
			<td align='left'>".@$fdep['NomCompleto']."</td>
