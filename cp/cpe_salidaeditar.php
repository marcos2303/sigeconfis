...
<table class="tblForm" width="895px" border="0">
  ...
  <tr>
	...
	<?php if(isset($fcon['FechaRegistro'])){ list($a, $m, $d)=preg_split( '/[.-]/', $fcon['FechaRegistro']); $f_registro=$d.'-'.$m.'-'.$a;}?>
   ...
   <?
     $sa = "select 
			 ...
			 where
				  ...
				  rp.CodCargo =  '".$fcon['Cargo']."' "; echo $sa;
		...
		if($risext!=0){
		  ...
		  for($i=0; $i<$risext; $i++){
			...
			if($fdisext['FlagEsParticular']=='N'){ 
             ...
             <td align="left" width="70"><input type="text" name="representante" id="representante" size="70" value="<?=utf8_encode($fdisext['Representante'])?>"/></td>
             
             <td align="left" width="70">
                 <input type="text" name="cargorepresentante"  size="60" id="cargorepresentante" value="<?=utf8_encode($fdisext['Cargo'])?>"/>
			 ...
			 <? }else{
			      if($fdisext['Cod_Dependencia']==''){?>
                   <tr class="trListaBody" onclick="mClk(this, 'seldetalle');" id="<?=$fdisext['Cod_Organismos']?>">
                     <td align="center" width="20"><input type="text" id="ente" name="ente" value="<?=utf8_encode($fc[0]);?>" size="70" />
                        <input type="hidden" id="codigo_organismo" name="cod_organismo" value="<?=$fdisext['Cod_Organismos']?>"/>
					 </td>
                     <td align="left" width="70"><input type="text" id="nombRepresentante" name="nombRepresentante" value="<?=utf8_encode($fc[1]);?>" size="70"/>
                          <input type="hidden" id="codigo_dependencia" name="codigo_dependencia" value=""/>
                     </td>
                     <td align="left" width="70"><input type="text" name="cargoRepresentanteLegal" value="<?=utf8_encode($fc[2]);?>" size="60"/>
                  ...
                  <? }else{?>
                     <tr class="trListaBody" onclick="mClk(this, 'seldetalle');" id="<?=a|$fdisext['Cod_Organismos']?>">
                       <td align="center" width="20"><input type="text" name="ente" id="ente" value="<?=utf8_encode($fc[0]);?>" size="70"/>
                              <input type="hidden" name="codigo_organismo" value="<?=$fdisext['Cod_Organismos']?>"/>
                       </td>
                       <td align="left" width="70"><input type="text" name="repre_dependencia" value="<?=utf8_encode($fc[1]);?>" size="70"/>
                              <input type="hidden" name="codigo_dependencia" value="<?=$fdisext['Cod_Dependencia']?>"/>
                       </td>
                       <td align="left" width="70"><input type="text" name="cargo_repredependencia" value="<?=utf8_encode($fc[2]);?>" size="60"/>
