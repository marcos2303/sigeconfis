...
<?
 ...
 $ano = substr($cod_documentocompleto, -4);
 $s="select * from cp_documentoextsalida where Cod_DocumentoCompleto='$cod_documentocompleto' and Periodo = '$ano'";
...
<form name="framemain" action="cpe_salidalista.php?limit=0&accion=guardarContenidoRestringido&fElaboradoPor=<?=$fElaboradoPor?>&fEstado=<?=$fEstado?>" method="post">
  ...
  <tr>
     ...
     <td width="296">
          <? 
			  ...
			  echo"<input type='text' id='fElaboradoPor' name='fElaboradoPor' value='".$fElaboradoPor."'/>";
     ...
     <td width="122" class="tagForm">Fecha Documento:</td>
         <? 
            if($f['FechaRegistro']!='0000-00-00'){list($a,$m,$d)=SPLIT('[/.-]',$f['FechaDocumento']);$f_documento=$d.'-'.$m.'-'.$a;} 
         ?>
 ...
 <tr>
   ...
   ?>
    ...
    <td><input name="coddependencia" type="hidden" id="coddependencia" value="<?=$fcon[2]?>"/>
	   <input name="dependencia" id="dependencia" type="text" size="68" value="<?=$fcon[3]?>" readonly/></td>
  </tr>
  <tr>
    <td class="tagForm">Representante:</td>
    <td><input name="codpersona" type="hidden" id="codpersona" value="<?=$_POST['registro']?>"/>
	 <input name="nomempleado" id="nomempleado" type="text" size="68" value="<?=$fcon[0]?>" readonly/></td>
  </tr>
  <tr>
    ...
    <tr>
      ...
      <table width="435px" class="tblLista">
	   ...
	   $sdisext="select 
				   ...
				   where 
				        Secuencia=(select max(Secuencia) from cp_documentodistribucionext where Cod_Documento='".$f['Cod_Documento']."' and 
																								 Periodo ='$ano' and 
																								 Cod_Tipodocumento='".$f['Cod_TipoDocumento']."' and 
																								 CodOrganismo='".$f['CodOrganismo']."') and 
						Cod_Documento='".$f['Cod_Documento']."' and 
					    Periodo ='$ano' and 
					    Cod_Tipodocumento='".$f['Cod_TipoDocumento']."' and 
		...
		if($risext!=0){
		  ...
		  for($i=0; $i<$risext; $i++){
			...
			if($fdisext['FlagEsParticular']=='N'){ 
		     ...
		     <tr>
             <td><?=utf8_encode($fc[0]);?></td>
             <td><?=utf8_encode($fc[1]);?></td>
		     </tr>
		     <? }
  ...
  <tr>
     <td colspan="4">
		<p>
		  <textarea class="ckeditor" cols="80" id="editor1" name="editor1" rows="10"><?=$f['Contenido']?></textarea>
		</p>
    </td>
  </tr>
</table>
<center>
    <input type="submit" value="Guardar Registro"/><input type="button" value="Cancelar" onClick="cargarPagina(this.form,'cpe_salidalista.php?limit=0');" />
</center>
</form>
</body>
</html>