...
elseif ($_POST['modulo']=="PERSONAS") {
	...
	list($dia, $mes, $annio)=preg_split( '[/.-]', @$_POST['fnac']); $fnac=$annio.$mes.$dia;
...
//// ELIMINAR DATOS MAESTROS CREADOS
//// _____________________________________________________
...
//// _____________________________________________________
if($_POST['accion']=="ELIMINARCOR"){
  $sql="DELETE FROM cp_tipocorrespondencia WHERE Cod_TipoDocumento='".$_POST['codigo']."'";
  $qry=mysql_query($sql) or die ($sql.mysql_error());
}
//// ---------------------------------------------------------------
if($_POST['accion']=="ELIMINARPARTICULAR"){
  $sql="DELETE FROM cp_particular WHERE CodParticular='".$_POST['codigo']."'";
  $qry=mysql_query($sql) or die ($sql.mysql_error());
}
//// ---------------------------------------------------------------
////             ELIMINAR DATOS DE CORRESPONDENCIA
$year=date("Y");
//// ---------------------------------------------------------------
if($_POST['accion']=="ELIMINARDOCUMENTOEXTERNO"){
  $scon="select Estado from cp_documentoextentrada where Cod_Documento='".$_POST['codigo']."' and Periodo='$year'";
  $qcon=mysql_query($scon) or die ($scon.mysql_error());
  if(mysql_num_rows($qcon)!=0){
    $fcon=mysql_fetch_array($qcon);
	if($fcon['Estado']=='PE'){
	  $sql="DELETE FROM cp_documentoextentrada WHERE Cod_Documento='".$_POST['codigo']."' and Periodo='$year'";
      $qry=mysql_query($sql) or die ($sql.mysql_error());
	}else{
	  echo"<script>";
	  echo"alert('¡El Registro no puede ser eliminado!')";
	  echo"<script/>";
	}
  }  
}
//// ---------------------------------------------------------------
if($_POST['accion']=="ELIMINARDOCUMENTOSALIDAEXTERNA"){
  $scon="select Estado from cp_documentointerno where Cod_Documento='".$_POST['codigo']."' and Periodo='$year'";
  $qcon=mysql_query($scon) or die ($scon.mysql_error());
  if(mysql_num_rows($qcon)!=0){
    $fcon=mysql_fetch_array($qcon);
	if(($fcon['Estado']=='PR') or ($fcon['Estado']=='PP')){
	  $sql="DELETE FROM cp_documentointerno WHERE Cod_Documento='".$_POST['codigo']."' and Periodo='$year'";
      $qry=mysql_query($sql) or die ($sql.mysql_error());
	  //// ELMININAMOS REGISTROS EN TABLA DISTRIBUCION
	  $sa="";
	}else{
	  echo"<script>";
	  echo"alert('¡El Registro no puede ser eliminado!')";
...
//// ---------------------------------------------------------------
//// ANULAR REGISTRO SALIDA DE DOCUMENTOS EXTERNOS - CORRESPONDENCIA
//// ---------------------------------------------------------------
if($_POST['accion']=="AnularRegistroSalidaExterna"){

 list($cod_tipodocumento, $cod_documentocompleto)=SPLIT('[|]', $_POST['codigo']);	
 
 $sa="select * from cp_documentoextsalida where Cod_DocumentoCompleto = '$cod_documentocompleto'";
 ...
 if(mysql_num_rows($qa)!=0){
   ...
   if($fa['Estado']=='PR'){
     $sb="update cp_documentoextsalida set Estado = 'AN',
	 									   FechaAnulado = '".date("Y-m-d")."',
	 									   MotivoAnulado = 'NO PROCEDE',
										   UltimoUsuario= '".$_SESSION['USUARIO_ACTUAL']."',
										   UltimaFechaModif = '".date("Y-m-d H:i:s")."'
									  where
									       Cod_DocumentoCompleto = '$cod_documentocompleto'";
    $qb=mysql_query($sb) or die ($sb.mysql_error());
    
    $sc = "update cp_documentodistribucionext set Estado = 'AN',
                                                  UltimoUsuario = '".$_SESSION['USUARIO_ACTUAL']."',
                                                  UltimaFechaModif = '".date("Y-m-d H:i:s")."'
                                             where 
                                                  Cod_Documento = '".$fa['Cod_Documento']."' and
                                                  Periodo = '".$fa['Periodo']."'";
    $qc = mysql_query($sc) or die ($sc.mysql_error());
   }else{
     if($fa['Estado']=='PP'){
	    $sb="update cp_documentoextsalida set Estado = 'PR',
										      UltimoUsuario= '".$_SESSION['USUARIO_ACTUAL']."',
										      UltimaFechaModif = '".date("Y-m-d H:i:s")."'
									     where
									          Cod_DocumentoCompleto = '$cod_documentocompleto'";
        $qb=mysql_query($sb) or die ($sb.mysql_error());
		 ...
		 $sc="update cp_documentodistribucionext set Estado = 'PR',
										             UltimoUsuario = '".$_SESSION['USUARIO_ACTUAL']."',
										             UltimaFechaModif = '".date("Y-m-d H:i:s")."'
									            where
									                 Cod_Documento = '".$fa['Cod_Documento']."' and
...
//// ---------------------------------------------------------------
//// ANULAR REGISTRO DOCUMENTOS INTERNOS - CORRESPONDENCIA
//// ---------------------------------------------------------------
if($_POST['accion']=="AnularRegistroInterno"){
 list($cod_tipodocumento, $cod_documentocompleto) = preg_split('/[|]+/',$_POST['codigo']);
 //echo "cod_tipodocumento=".$cod_tipodocumento;	
 //echo "cod_documentocompleto=".$cod_documentocompleto;	
 ...
 if(mysql_num_rows($qa)!=0){
   ...
   if($fa['Estado']=='PR'){
     //// ANULA DE MANERA DIRECTA EL DOCUMENTO 
     $sb="update cp_documentointerno set Estado = 'AN',
	 									   FechaAnulado = '".date("Y-m-d")."',
	 									   MotivoAnulado = 'NO PROCEDE',
										   UltimoUsuario= '".$_SESSION['USUARIO_ACTUAL']."',
										   UltimaFechaModif = '".date("Y-m-d H:i:s")."'
	...
	//// ANULA DISTRIBUCION DIRECTAMENTE 
	$sc="update cp_documentodistribucion set Estado = 'AN',
										     UltimoUsuario= '".$_SESSION['USUARIO_ACTUAL']."',
										     UltimaFechaModif = '".date("Y-m-d H:i:s")."'
									    where
    ...
    $qc=mysql_query($sc) or die ($sc.mysql_error());
     ...
     if($fa['Estado']=='PP'){
	  //// CAMBIA EL ESTADO DE PREPARADO A PREPARACION AL DOCUMENTO
	    $sb="update cp_documentointerno set Estado = 'PR',
										    UltimoUsuario= '".$_SESSION['USUARIO_ACTUAL']."',
										    UltimaFechaModif = '".date("Y-m-d H:i:s")."'
									   where
									        Cod_DocumentoCompleto = '$cod_documentocompleto' and 
	  ...
	  //// CAMBIA EL ESTADO DE LA DISTRIBUCION DEL DOCUMENTO DE PENDIENTE A PREPARACION
		 $sc="update cp_documentodistribucion set Estado = 'PR',
										          UltimoUsuario= '".$_SESSION['USUARIO_ACTUAL']."',
										          UltimaFechaModif = '".date("Y-m-d H:i:s")."'
									         where
									              Cod_Documento = '$cod_documentocompleto' and 
