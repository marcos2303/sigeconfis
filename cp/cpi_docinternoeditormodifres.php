...
<table width="100%" cellspacing="0" cellpadding="0">
	<tr>
		...
		<td align="right"><a class="cerrar" href="framemain.php"  onclick="cargarPagina(document.getElementById('frmentrada'),'cpi_docinternoslista.php?&limit=0')";>[Cerrar]</a></td>
	</tr>
</table><hr width="100%" color="#333333" />
<form name="framemain" action="cpi_docinternoslista.php?limit=0&fremitente=<?=$fremitente?>&accion=guardarContenidoInternoRestringido" method="post">
...
<tr> <!-- ///////////  PRIMER ////////// -->
  ...
  <tr>
	 ...
	 ?>
      <input type="hidden" name="cod_tipodocumento" id="cod_tipodocumento" value="<?=$field['Cod_TipoDocumento'];?>"/> <input name="t_documento" id="t_documento" value="<?=$field['Descripcion']?>" size="20"  readonly/>   </td>
...
<tr>
 ...
 <tr>
    ...
    <td><input name="coddependencia" type="hidden" id="coddependencia" value="<?=$fcon[2]?>"/>
	   <input name="dependencia" id="dependencia" type="text" size="68" value="<?=$fcon['Dependencia']?>" readonly/></td>
  </tr>
    <tr>
     <td class="tagForm">Representante:</td>
     <td><input name="codpersona" type="hidden" id="codpersona" value="<?=$_POST['registro']?>"/>
         <input name="nomempleado" id="nomempleado" type="text" size="68" value="<?=$fcon['NomCompleto']?>" readonly/>
         </td>
    </tr>
    <tr>
     <td class="tagForm">Cargo:</td>
     <td><input type="text" id="cargoremit" name="cargoremit" value="<?=$fcon['DescripCargo']?>"size="68"  readonly/></td>
    </tr>
    <tr>
      <td class="tagForm">Iniciales:</td>
      <td><input type="text" id="iniciales" name="iniciales" size="5" maxlength="5" value="<?=$f['MediaFirma']?>"/></td>
    </tr>
    </table>
   </td>
   <td>
    ...
    <td>
	   ...
	   $risext= mysql_num_rows($qdisext); //echo $risext;
		    ...
		    $fdisext = mysql_fetch_array($qdisext);
			   $sa="select
                      ...
                      from
						  ...
						  rh_puestos rh
                     where 
                          mp.CodPersona='".$fdisext['CodPersona']."' and 
						  md.CodDependencia = '".$fdisext['CodDependencia']."' and 
						  rh.CodCargo = '".$fdisext['CodCargo']."'";
						
			   $qa = mysql_query($sa) or die ($sa.mysql_error());
			   if (mysql_num_rows($qa) != 0) $fa = mysql_fetch_array($qa); 
			   
			  //// **********************
			   $se = "select max(Secuencia) from rh_empleadonivelacion where CodPersona = '".$fa['CodPersona']."'"; 
			   ...
			   $sf = "select 
						...
						where 
							 rh.CodPersona = '".$fa['CodPersona']."' and 
							 rh.Secuencia = '".$fe['0']."'"; //echo $sf;
		...
		?>
         <tr>
             <td><?=$fa['Dependencia']?></td>
             <td><?=$fa['NomCompleto'];?></td>
             <td><?=$ff['DescripCargo'];?></td>
         </tr>
        <?
         }
		 }
...
<tr>
  <td colspan="4"><textarea class="ckeditor" cols="80" id="editor1" name="editor1" rows="10"><?=$f['Contenido']?></textarea>
  </td>
</tr>
</table>
<center>
<input type="submit" value="Guardar Registro"/><input type="button" value="Cancelar" onClick="cargarPagina(this.form,'cpi_docinternoslista.php?limit=0&fEstado=<?=$fEstado?>&fremitente=<?=$fremitente?>');" />
</center>
</form>
</body>
</html>