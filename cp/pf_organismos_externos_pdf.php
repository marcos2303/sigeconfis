...
function Cabecera($pdf){
	...
	$pdf->Cell(190, 10,utf8_decode('Maestro Organismos Externos'), 0, 1, 'C');	
	$pdf->SetDrawColor(0, 0, 0); $pdf->SetFillColor(200, 200, 200); $pdf->SetTextColor(0, 0, 0);
	$pdf->SetFont('Arial', 'B', 6);
	$pdf->Cell(20, 5, 'COD. ORGANISMO', 1, 0, 'C', 1);
	$pdf->Cell(80, 5, 'ORGANISMO', 1, 0, 'C', 1);
	$pdf->Cell(80, 5, 'REPRESENTANTE LEGAL', 1, 0, 'C', 1);
...
while ($field=mysql_fetch_array($query)) {
	...
	$pdf->SetFont('Arial', '', 6);
	$pdf->SetWidths(array(20, 80, 80, 15));
	$pdf->SetAligns(array('C', 'L', 'L', 'C'));
	$pdf->Row(array($field['CodOrganismo'], $field['Organismo'], $field['RepresentLegal'], $estado));
