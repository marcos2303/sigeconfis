...
<table width="800" class="tblLista">
  <tr class="trListaHead">
	...
	if ($filtro!="") $sql="SELECT CodAplicacion, Descripcion, PrefVoucherPD, PeriodoContable,Estado FROM mastaplicaciones WHERE (CodAplicacion LIKE '%$filtro%' OR Descripcion LIKE '%$filtro%' OR PeriodoContable LIKE '%$filtro%' OR Estado LIKE '%$filtro%' OR PrefVoucherPD LIKE '%$filtro%') ORDER BY CodAplicacion";
	else $sql="SELECT CodAplicacion, Descripcion, PeriodoContable, PrefVoucherPD, Estado FROM mastaplicaciones ORDER BY CodAplicacion";
	$query=mysql_query($sql) or die ($sql.mysql_error());
	...
	for ($i=0; $i<$rows; $i++) {
		...
		<tr class='trListaBody' onclick='mClk(this, \"registro\");' onmouseover='mOvr(this);' onmouseout='mOut(this);' id='".$field['CodAplicacion']."'>
			...
			<td align='center'>".$field['PrefVoucherPD']."</td>
