	  ...
	  // Condiciones para especificar datos de particulares y organismos
		...
		$scon4 = "select 
		 ...
		 for($b=0;$b<$rcon4;$b++){
			...
			// --------------------------------------------------------
			  if($fcon4['Estado']=='RE'){ @list($ano,$mes,$dia)=preg_split('/[-]/',$fcon4['FechaRecibido']); $fechaMovimiento = $dia.'-'.$mes.'-'.$ano;
			  }else{ 
			  }if($fcon4['Estado']=='DV'){ @list($ano,$mes,$dia)=preg_split('/[-]/',$fcon4['FechaDevolucion']); $fechaMovimiento = $dia.'-'.$mes.'-'.$ano;
			  }else{
			      @list($ano,$mes,$dia)=preg_split('/[-]/',$fcon4['FechaEnvio']);
		  ...
		  if(($fcon2['FlagEsParticular']=='N')and($bandera=='1')){
		    ...
		    $pdf->Row(array($fcon['Cod_Documento'],$fcon3['Organismo'],$fcon3['Dependencia'],utf8_decode($fcon2['Representante']),$fcon2['Cargo'],$estadoActual,$fechaMovimiento));
		  }else{
		    ...
		    $pdf->SetAligns(array('C','C','C','L', 'L','C','C'));
		    $pdf->Row(array($fcon['Cod_Documento'],'Particular','Particular',utf8_decode($fcon2['Representante']),$fcon2['Cargo'],$estadoActual,$fechaMovimiento));
			}else{
