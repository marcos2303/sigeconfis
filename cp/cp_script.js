...
function guardarDocumentoInterno(form) { 
	...
	var cargodestinatario_int = document.getElementById("cargodestinatario_int").value;
	
	if(form.encarg_especial.checked) var FlagEncargaduriaEspecial='S';
	...
	for(i=0; n=frmdetalles.elements[i]; i++) {
		...
		if (n.name == "dependencia") detalles += n.value + "|";
		if (n.name == "cargo") detalles += n.value + "|";
		if (n.name == "descp_cargo") detalles += n.value + ";";
	}
...
function guardarDocumentoInternoEditar(form) {
	...
	if (error_detalles != "") alert(error_detalles);
		...
		ajax.open("POST", "gmcorrespondencia.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("accion=guardarDocumentoInternoEditar&detalles="+detalles+"&organismo="+organismo+"&dep_interna="+dep_interna+"&t_documento="+t_documento+"&n_documento="+n_documento+"&asunto="+asunto+"&descrip="+descrip+"&destinatario_int="+destinatario_int+"&codigo_interno="+codigo_interno+"&codigo_persona="+codigo_persona+"&codigo_cargo="+codigo_cargo+"&plazo="+plazo+"&anexsi1="+anexsi1+"&anexsi2="+anexsi2+"&anexDescp="+anexDescp+"&Estado="+Estado+"&Anexos="+Anexos+"&FlagEncargaduriaEspecial="+FlagEncargaduriaEspecial+"&cargodestinatario_int="+cargodestinatario_int);
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4)	{
				var resp = ajax.responseText;
				if (resp != "") alert(resp.trim());
...
function insertarDestinatarioDep(codigo, accion) { //alert('Pasasssss');
	...
	ajax.open("POST", "gmcorrespondencia.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send("accion=insertarDestinatarioDep&codigo="+codigo+"&ventana="+ventana+"&tabla="+tabla+"&detalles="+detalles+"&contador="+contador);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4)	{
			var resp = ajax.responseText;
			...
			if (datos[0].trim() != "") ;
				...
				opener.document.getElementById("contador").value = contador;
				window.close();
			}
		}
	}
...
//// ---------------------------------------------------------------------------------------
function prueba(id){ //alert('id p= '+id);

 	document.getElementById("id_descp_cargo").value = id; ///alert("id_descp_cargo= "+document.getElementById("id_descp_cargo").value);
	
    var id = id.split("_");
	var id01 = id[0]; //alert('id01='+id01);
	var id02 = id[1]; //alert('id02='+id02);
    
	document.getElementById("id_cargo").value = "cargo_"+id02; ///alert("id_cargo= "+document.getElementById("id_cargo").value);
	
}
//// ---------------------------------------------------------------------------------------
function insertarCargos(id){ 
  
  var id = id.split("|");
  var id01 = id[0]; //alert('id01='+id01);
  var id02 = id[1]; //alert('id02='+id02);
  
  var Activar = document.getElementById("Activar").value; //alert('Activar= '+Activar);
  if(Activar == 'D'){
    var codigo_cargo  = document.getElementById("codigo_cargo").value; //alert('codigo_cargo='+codigo_cargo);
	opener.document.getElementById(codigo_cargo).value = id01; 
	opener.document.getElementById('cargodestinatario_int').value = id02;
  }else{ 
    var id_cargo = document.getElementById("id_cargo").value; //alert('id_cargo='+id_cargo);
    var id_descp_cargo = document.getElementById("id_descp_cargo").value;	///alert('id_descp_cargo='+id_descp_cargo); 
    opener.document.getElementById(id_cargo).value  = id01; ///alert('id cargo=  '+document.getElementById("id_cargo").value);
    opener.document.getElementById(id_descp_cargo).value = id02; ///alert('id descp cargo = '+document.getElementById("id_descp_cargo").value);
  }
...
//// ---------------------------------------------------------------------------------------
//// 			FUNCION QUE PERMITE ACTIVAR BOTON PARA SALECCION DE CARGO
//// ---------------------------------------------------------------------------------------
function botonCargo(form){
  if(form.encarg_especial.checked){
	   /*frmdetalles.btCargo.style.visibility='visible';*/
	   form.btCargoDest.style.visibility ='visible';
  }else{ 
       /*frmdetalles.btCargo.style.visibility='hidden';*/
	   form.btCargoDest.style.visibility ='hidden';
  }
}
//// ---------------------------------------------------------------------------------------
////        FUNCION QUE PERMITE INSERTAR UNA DEPENDENCIA " MODULO DE CORRESPONDENCIA"
//// ---------------------------------------------------------------------------------------
/*function insertarCargos(codigo, accion) {
	 //alert('Pasasssss');
    var cod = codigo.split("|");
	var codigo01 = cod[0]; //alert('cod1='+cod1);
	var codigo02 = cod[1]; //alert('cod2='+cod2);
	...
	ajax.open("POST", "gmcorrespondencia.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send("accion=insertarCargo&codigo="+codigo+"&ventana="+ventana+"&tabla="+tabla+"&detalles="+detalles);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4)	{
			var resp = ajax.responseText;
			var datos = resp.split("||");
