...
if(!isset($cRemitente) or $cRemitente == '') $cRemitente = '';
if(!isset($cProcedencia) or $cProcedencia == '') $cProcedencia = '';
...
if(!isset($fEstado) or $fEstado == '') $fEstado = '';
if(!isset($fechaRecibido) or $fechaRecibido == '') $fechaRecibido = '';
if(!isset($f_Remitente) or $f_Remitente == '') $f_Remitente = '';
if(!isset($fRecibidoPor) or $fRecibidoPor == '') $fRecibidoPor = '';
if(!isset($fProcedencia) or $fProcedencia == '') $fProcedencia = '';
//	------------------------------------
include("fphp.php");
connect();
//	------------------------------------
...
$sql="SELECT 
	...
	ORDER BY 
	          Periodo, Cod_DocumentoCompleto"; //echo $sql;
...
if ($registros!=0) {
	for ($i=0; $i<$rows; $i++) {
		...
		if($field['FechaRegistro']=='0000-00-00'){$f_registro='';}else{ list($a, $m, $d)=preg_split( '/[.-]/', @$field['FechaRegistro']); $f_registro=$d.'-'.$m.'-'.$a;}
		if($field['FechaDocumento']=='0000-00-00'){$f_documento='';}else{ list($a, $m, $d)=preg_split( '/[.-]/', @$field['FechaDocumento']); $f_documento=$d.'-'.$m.'-'.$a;} 
		...
		<tr class='trListaBody' onclick='mClk(this, \"registro\");' id='$id'>
		    ...
		    <td align='center'>".$fdist['NomCompleto']."</td>
...
if(!isset($_UPDATE) or $_UPDATE == '') $_UPDATE = '';
