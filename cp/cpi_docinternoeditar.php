...
<?php
 list($cod_tipodocumento, $cod_documentocompleto)=preg_split( "/[|]+/",$_POST['registro']);
...
<tr>
 ...
 <? list($a, $m, $d)=preg_split( "/[.-]+/", $f['FechaDocumento']); $f_documento=$d.'-'.$m.'-'.$a; ?>
...
<tr><td height="5"></td></tr>
 <tr>
    ...
    <td colspan="2"><select id="organismo" name="organismo" class="selectBig" onchange="getOrganismoInterno(this.id, 'dep_interna');" disabled>
      <option value=""></option>
      <? getOrganismos(0, $_SESSION['ORGANISMO_ACTUAL']); ?>
    </select>
      *</td><? 
	          if($f['FlagEncargaduriaEspecial']=='S'){
				  $est = "checked";
	        ...
	        ?>
      <td><input type="checkbox" id="encarg_especial" name="encarg_especial" <?=$est;?> onclick="botonCargo(this.form);"/> Encargadur&iacute;a Especial</td>
  </tr>
  <tr>
    <td class="tagForm" colspan="">Dependencia:</td>
    <td colspan="2"><select id="dep_interna" name="dep_interna" class="selectBig" disabled>
      <option value=""></option>
  ...
  <tr>
    ...
    <td><input type="text" id="destinatario_int" name="destinatario_int" size="68" value="<?=$fa['Representante']?>" readonly/></td>
    ...
    <td><input type="text" id="cargodestinatario_int" name="cargodestinatario_int" size="68" value="<?=$fa['Cargo']?>" readonly/>
        ...
        <input type="hidden" id="codigo_cargo" name="codigo_cargo" value="<?=$f['Cod_CargoRemitente']?>"/>
        <input type="button" id="btCargoDest" name="btCargoDest" value='Cargo' onclick="cargarVentana(this.form,'lista_cargos.php?limit=0&ventana=insertarCargos&Activar=D&codigo_cargo=codigo_cargo', 'height=600, width=800, left=50, top=50, resizable=yes');" <?=$visible;?>/></td>
  </tr>
<tr><td height="5"></td></tr>
<tr>
 <td colspan="4">
...
<input type="hidden" id="valor_campos" name="valor_campos"/>

<input type="hidden" id="id_descp_cargo" name="id_descp_cargo"/>
...
<tr>
  ...
  <table width="800" class="tblBotones">
   <tr>
 	<td align="right">
    <input type="button" class="btLista" id="btCargo" name="btCargo" value="Cargo" onclick="Verificar()|cargarVentana(this.form,'lista_cargos.php?limit=0&ventana=insertarCargos&tabla=item&id_descp_cargo='+document.getElementById('id_descp_cargo').value+'&id_cargo='+document.getElementById('id_cargo').value, 'height=600, width=800, left=50, top=50, resizable=yes');" <?=$visible;?> /> 
	   ...
	   for($i=0; $i<$rdist; $i++){
		 ...
		 if (mysql_num_rows($qc) != 0) $fc = mysql_fetch_array($qc);
		     ...
		     <tr class="trListaBody" onclick="mClk(this, 'seldetalle');" id="<?=$fc['3']?>">
                <td><input type="hidden" name="codpersona" value="<?=$fc[3]?>"/>
				    <input type="hidden" name="cod_dependencia" value="<?=$fc[4]?>"/>
                    <input type="text" size="100" name="dependencia" id="dependencia" value="<?=htmlentities(@$fc[0]);?>" readonly/></td>
                    
                <td><input type="text" name="nombcompleto" id="nombcompleto" value="<?=htmlentities(@$fc[1]);?>" size="100" readonly/></td>
                
                <td><input type="hidden" id="contador" value="<?=@$contador;?>"/>
                    <input type="hidden" name="cargo" value="<?=@$fc[5]?>"/> 
					<input type="text" name="descp_cargo" id="descp_<?=@$contador;?>" value="<?=htmlentities(@$fc[2]);?>" size="100" readonly  onclick="asumoInsert(this.id)|prueba(this.id);"/></td>
		     </tr>
             <?
		 }
	   }
