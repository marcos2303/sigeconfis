...
if(!isset($fremitente) or $fremitente =='') $fremitente = '';
...
if(!isset($dTDocumento) or $dTDocumento =='') $dTDocumento = '';
...
if ($registros!=0) {
	...
	//// ------------------------------------------------------------------------	
		...
		list($a, $m, $d)=preg_split( '/[.-]/', @$field['FechaDistribucion']); $f_distribucion=$d.'-'.$m.'-'.$a;
		...
		<tr class='trListaBody' onclick='mClk(this, \"registro\");'>
			...
			<td align='center'>".@$fdep['Dependencia']."</td>
