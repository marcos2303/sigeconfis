...
echo "
<form name='frmentrada' action='cpe_salidalista.php?limit=0&concepto=$concepto' method='POST'>
...
<input name="btEliminar" type="button" class="btLista" id="btAnular" value="Anular" onclick="AnularRegistroSalidaExterna(this.form, 'cpe_salidalista.php?limit=0', '1', 'PERSONAS');"/>
...
if ($registros!=0) {
	for ($i=0; $i<$rows; $i++) {
		...
		list($a, $m, $d)=preg_split( '/[.-]/', $field['FechaRegistro']); $f_elaborado=$d.'-'.$m.'-'.$a;
		...
		$id = $field['Cod_TipoDocumento'].'|'.$field['Cod_DocumentoCompleto'].'|'.$field['Periodo'];
		//// ____________________________________________________________________________________________
		echo "
