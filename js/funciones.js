...
function insertarLinea2(boton, accion, detalle, php, valores) {
	...
	*/
	...
	var candocumentos = $("#cuenta_documentos").val();
	$("#cuenta_documentos").val(parseInt(candocumentos) + 1);
	//	defino el path
	if (!php) var php_ajax = "../lib/fphp_funciones_ajax.php";
	else var php_ajax = "lib/fphp_funciones_ajax.php";
	
	//alert('accion="'+accion+'"&nrodetalle="'+nrodetalle+'"&candetalle"'+candetalle+'"&"' +valores);
	//	ajax
	$.ajax({
		type: "POST",
		url: php_ajax,
...
function verificaDocumentos(boton, detalle)
{
	var codActividad = $('#sel_actividades').val();
	var codActuacion = $('#CodActuacion').val();
	boton.disabled = true;
	var can = "#can_" + detalle;
	var sel = "#sel_" + detalle;	
	var lista = "#lista_" + detalle;
	if ($(sel).val() == "") cajaModal("Debe seleccionar una linea", "error", 400);
		...
		$.ajax({
			type: "POST",
			url: "pf_documentos_controller.php",
			data: "action=list&CodActuacion=" + codActuacion + "&CodActividad=" + codActividad ,
			async: false,
			success: function(resp){
				cajaModal(resp, "", '100%');
			}
...
function actualizar_afecto_retenciones(MontoAfecto, MontoNoAfecto, MontoImpuesto, frm_impuesto) {
	...
	for(var i=0; n=frm_impuesto.elements[i]; i++) {
		if (n.name == "Signo") var _Signo = n.value;
			...
			if (_FlagImponible == "I") var _MontoAfecto = parseFloat(MontoImpuesto);
			else if (_FlagImponible == "N") var _MontoAfecto = parseFloat(MontoAfecto);
			else if (_FlagImponible == "B") var _MontoAfecto = parseFloat(MontoAfecto + MontoNoAfecto);
			n.value = setNumeroFormato(_MontoAfecto, 2, ".", ",");
		}
		else if (n.name == "FactorPorcentaje") var _FactorPorcentaje = parseFloat(setNumero(n.value));
		else if (n.name == "MontoImpuesto") {
			var _MontoImpuesto = parseFloat(_MontoAfecto * _FactorPorcentaje / 100);
...
//	--
function getDescripcionLista2(accion, campo, descripcion, campo2, descripcion2, campo3, descripcion3) {
	var codigo = campo.value;
	if (codigo.trim() != "") {
		$.ajax({
			type: "POST",
			...
			success: function(resp) {
				var datos = resp.split("|");
				if (campo) campo.value = datos[1];
				if (descripcion) descripcion.val(datos[2]);
				if (campo2) campo2.val(datos[3]);
				if (descripcion2) descripcion2.val(datos[4]);
				if (campo3) campo3.val(datos[5]);
				if (descripcion3) descripcion3.val(datos[6]);
			}
		});
	} else {
		campo.value = "";
