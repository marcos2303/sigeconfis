	...
	elseif ($accion == "aprobar") {
		...
		$sql = "INSERT INTO rh_empleadonivelacion
				SET
					CodPersona = '".$CodPersona."',
					Secuencia = '".$Secuencia."',
					Fecha = '".formatFechaAMD($FechaEgreso)."',
					CodOrganismo = '".$CodOrganismo."',
					CodDependencia = '".$CodDependencia."',
					CodCargo = '".$CodCargo."',
					CodTipoNom = '".$CodTipoNom."',
		...
		if ($Tipo == "C") {
			...
			$campos_dias = "";
			$inicio = getFechaDias(formatFechaDMA($field_ba['FechaInicio']), $FechaEgreso);
			$DiasPago = 0;
			$DiasFeriados = 0;
			$DiasInactivos = 0;
...
elseif ($modulo == "disfrute_vacacionales") {
	...
	elseif ($accion == "eliminar") {
		...
		//	-----------------
		//	elimino
		$sql = "DELETE FROM rh_vacaciontabla WHERE NroAnio = '".$registro."'";
		$query_delete = mysql_query($sql) or die(getErrorSql(mysql_errno(), mysql_error(), $sql));
		//	-----------------
		mysql_query("COMMIT");
	}
