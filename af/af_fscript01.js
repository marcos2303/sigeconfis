...
function guardarAprobarMovimientoActivo(form){
 ...
 /// VALORES MOVIMIENTO ANTERIOR
 var CentroAnterior = document.getElementById("c_costos").value;
 var UbicacionAnterior = document.getElementById("ubicacion").value;
 var EmpleadoAnterior = document.getElementById("e_usuario").value;
 var EmpleadoResponsableAnterior = document.getElementById("e_responsable").value;
 var CodDependenciaAnterior = document.getElementById("dependencia").value;
 var CodOrganismoAnterior = document.getElementById("organismo").value;
 
 /// VALORES MOVIMIENTO ACTUAL
 ...
 var Pase = 'P';
     ...
     ajax.onreadystatechange=function() {
		if (ajax.readyState==4)	{
			var resp = ajax.responseText.trim();
			if (resp != ""){ //alert(resp); 
			    if(resp==1){alert("EL USUARIO QUE PREPARO EL MOVIMIENTO DEBE SER DISTINTO AL MOMENTO DE APROBAR"); window.close();}
	            if(resp==2){alert("MOVIMIENTO APROBADO");
				  opener.document.getElementById("frmentrada").submit();
				  window.open('af_rptransferenciaactivofijo.php?Activo='+Activo+'&Organismo='+Organismo+'&MovimientoNumero='+MovimientoNumero,'', 'height=500, width=870, left=200, top=100, resizable=yes');
			      window.open('af_actaentregabm.php?Activo='+Activo+'&CodOrganismo='+Organismo+'&Pase='+Pase,'', 'height=500, width=870, left=200, top=100, resizable=yes');
				  window.close();
				}
			}//else{
...
function generarVoucherBaja(form, valor){ 
   ...
   var ajax=nuevoAjax();
	 ajax.open("POST","gmactivofijo.php", true);
	 ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	 ajax.send("accion="+valor+"&Activo="+Activo+"&CodTransaccionBaja="+CodTransaccionBaja+"&CodVoucher="+CodVoucher+"&CodContabilidad="+CodContabilidad+"&Periodo="+Periodo+"&CodOrganismo="+CodOrganismo+"&CodLibroCont="+CodLibroCont+"&CentroCosto="+CentroCosto);
	 ajax.onreadystatechange=function() {
		if (ajax.readyState==4)	{
			var error=ajax.responseText;
			if (error!=0) alert ("¡"+error+"!");
