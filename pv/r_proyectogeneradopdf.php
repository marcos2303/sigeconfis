...
function Cabecera($pdf) {
	...
	$pdf->SetXY(20, 15); $pdf->Cell(190,5,utf8_decode('División de Administración'), 0, 1, 'L');	
	$qry=mysql_query("SELECT Sector,
	                         Programa,
	                         SubPrograma,
							 Proyecto,
							 Actividad,
	                    ...
	                    FROM 
						     pv_antepresupuesto 
					   WHERE 
					         CodAnteproyecto='".$_GET['registro']."'") or die ($sql.mysql_error());
	$field=mysql_fetch_array($qry);
	list($a, $m, $d)=SPLIT( '[/.-]', $field[10]); $fechaGenerado=$d.'-'.$m.'-'.$a;
	$m_presupuestado=number_format($field['9'],2,',','.');
	...
	$pdf->Cell(140, 10, utf8_decode('Descripción de Anteproyecto'), 0, 1, 'C');	
	$pdf->SetFont('Arial', '', 7);
	...
	$pdf->Cell(55, 3, 'FECHA GENERADO:', 0, 0, 'R');$pdf->Cell(3, 3, $fechaGenerado, 0, 1, 'L');
