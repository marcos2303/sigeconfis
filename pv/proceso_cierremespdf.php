...
for($i=0; $i<$rows ; $i++){
 ...
 $montoPagado1 = 0; $montoPagadoTotal1 = 0; $montoDisPresu = 0; $montoReintegro = 0; $montoDisFinan = 0;
 ...
 if(($fieldet['partida']!=00) and (($cont1==0) or ($pCapturada!=$fieldet['partida']))){
  ...
  if($rwPar!=0){
   ...
   //// -------------------------------------------------------------------------
	...
	$montoPptoAjustado1 = number_format($montoPptoAjustado1,2,',','.');
	$montoDisponPresupuestaria1 = number_format($montoDisponPresupuestaria1,2,',','.');
	if($montoDisponPresupuestaria1 =='-0,00') $montoDisponPresupuestaria1 = '0,00';
	
	$montoDisponFinanciera1 = number_format($montoDisponFinanciera1,2,',','.');
	if($montoDisponFinanciera1 =='-0,00') $montoDisponFinanciera1 = '0,00';
 ...
 $montoPagado2 = 0; $montoPagadoTotal2 = 0;	 $montoDispPresu2 = 0; $montoDispFinan2 = 0;
 $montoDisponFinanciera2=0;  $montoDisponPresupuestaria2=0;
 if(($fieldet['partida']!=00) and (($cont2==0) or ($gCapturada!=$fieldet['generica']) or ($pCapturada2!=$fieldet['partida']))){
    $sql2="SELECT cod_partida,partida1,denominacion,cod_tipocuenta,generica,tipo 
			    FROM pv_partida 
			   WHERE partida1='".$fieldet['partida']."' AND
	...
	if($rows2!=0){
	  ...
	  //// ------------------------------------------------------------------
	   ...
	   $s_consulta02 = "select 
					  ...
					  from 				   
							pv_ajustepresupuestario aj
                            inner join pv_ajustepresupuestariodet ajdet on ((aj.CodPresupuesto=ajdet.CodPresupuesto)and(aj.CodAjuste=ajdet.CodAjuste)and
																			(aj.Organismo=ajdet.Organismo))
					where 
					       aj.CodPresupuesto = '".$fieldet['CodPresupuesto']."' and 
						   aj.Periodo = '$Periodo' and 
						   aj.Estado = 'AP' and
      ...
      for($c=0; $c<$r_consulta02; $c++){	
	     $f_consulta02 = mysql_fetch_array($q_consulta02); 
	     if($f_consulta02['TipoAjuste']=='IN'){
	       $montoIncremento2+= $f_consulta02['MontoAjuste'];
	     }else{ 
	       $montoDisminucion2 = $f_consulta02['MontoAjuste'] + $montoDisminucion2;
		 }
	  }
	  ...
	  /// ************ Calculo Monto Ajustado / Disponibilidad Presupuestaria
	   ...
	   $montoDisponPresupuestaria2 = $montoPptoAjustado2 - $montoCompromiso2; //echo $montoDisponPresupuestaria2.'**';
	   if($montoDisponPresupuestaria2=='-0,00') $montoDisponPresupuestaria2= 0;
	   $montoDisponFinanciera2 = (($montoDispFinan2 - $montoDispPresu2) + $montoPptoAjustado2) - $montoPagado2;
	   if($montoDisponFinanciera2=='-000')$montoDisponFinanciera2= 0;
	   $montoVariacion2 = $montoDisponFinanciera2 - $montoDisponPresupuestaria2;
	   
	   $montoPptoAjustado2 = number_format($montoPptoAjustado2,2,',','.'); 
	   $montoDisponPresupuestaria2 = number_format($montoDisponPresupuestaria2,2,',','.'); 
	   if($montoDisponPresupuestaria2=='-0,00') $montoDisponPresupuestaria2='0,00';
	   
	   $montoDisponFinanciera2 = number_format($montoDisponFinanciera2,2,',','.');
	   if($montoDisponFinanciera2=='-0,00') $montoDisponFinanciera2='0,00';
	   
	   $montoVariacion2 = number_format($montoVariacion2,2,',','.');
	   if($montoVariacion2=='-0,00') $montoVariacion2='0,00';
	  /// ************ 
	  ...
	  $montoDisponPresupuestaria2= "";
	  $montoDisponFinanciera2 =  "";
   }
  }
 //// ----------------------------------------------------------------------------------------------- */
 //// **** **** **** Capturando Partida Tipo "D" 301-01-01-00 ó 301-01-01-01
 //// ----------------------------------------------------------------------------------------------- */
 $montoIncremento3 = 0; $montoDisminucion3 = 0; $montoIncrementoTotal3 = 0; $montoDisminucionTotal3 = 0; 
 $montoCompromisoTotal = 0; $montoCompromiso3 = 0; $montoCausado3 = 0; $montoCausadoTotal3 = 0;
 $montoPagado3 = 0; $montoPagadoTotal3 = 0; $montoDispPresu3 = 0;  $montoDispFinan3 = 0;
 ...
 if($fieldet['partida']!=00){
	 ...
	 /// ************************************************************************* 
	   ...
	   for($c=0; $c<$r_pagado3; $c++){
	     ...
	     elseif(($f_pagado3['cod_partida']==$cp03)and($f_pagado3['Periodo']==$pr03)){ 
		       $r_03 =  -1*($f_pagado3['Monto']);
			   $cp03 = $f_pagado3['cod_partida']; 
			   $pr03 = $f_pagado3['Periodo']; 
			   $paso03 = 1 + $paso03;
			   $reintegro3 = $reintegro3 + $r_03; //echo 'Reintegro3= '.$reintegro3.'**';
			   $reintegroT = $reintegroT + $r_03; //echo 'Reintegro total= '.$reintegroT.'**';
	         }elseif(($f_pagado3['cod_partida']!=$cp03)or($f_pagado3['Periodo']!=$pr03)){ 
			   $r_03 =  -1*($f_pagado3['Monto']);
			   $cp03 = $f_pagado3['cod_partida']; 
			   $pr03 = $f_pagado3['Periodo']; 
			   $paso03 = 1 + $paso03;
	 ...
	 /// *************************************************************************
	 ///   ************ Calculo Monto Ajustado / Disponibilidad Presupuestaria
	   $montoPptoAjustado3 = ($f_ejecucion03['DispPresupuestaria'] + $reintegro3 + $montoIncremento3) - $montoDisminucion3; //echo $montoPptoAjustado3.'*'.$reintegro3.'*';
	   $montoPptoAjusTotal = $montoPptoAjusTotal + $montoPptoAjustado3; //echo $montoPptoAjusTotal.'/';
	
	/// ************************************************************************* 
	///                  Disponibilidad Presupuestaria
	   $montoDisponPresupuestaria3 = $montoPptoAjustado3 - $montoCompromiso3;
	   if($montoDisponPresupuestaria3<0)$montoDisponPresupuestaria3= $montoDisponPresupuestaria3*(-1); 
	   //echo $montoDisponPresupuestaria3;}
	   $montoDisponPresupuestTotal = $montoDisponPresupuestTotal + $montoDisponPresupuestaria3;
    /// ************************************************************************* 
	///                  Disponibilidad Financiera
	   ...
	   if($montoDisponFinanciera3<0)$montoDisponFinanciera3=$montoDisponFinanciera3*(-1);
	   $montoDisponFinancieraTotal = $montoDisponFinancieraTotal + $montoDisponFinanciera3;
	/// ************************************************************************* 
	///                      Monto Variación   
	   $montoVariacion3 = $montoDisponFinanciera3 - $montoDisponPresupuestaria3;
	...
	///// *** Mostrar *** /////
	//$montoT=number_format($montoT,2,',','.');
	$reintegroTotal = number_format($reintegroT,2,',','.'); //echo $reintegroTotal; /// Reintegro Total
...
$pdf->SetDrawColor(0, 0, 0); $pdf->SetFillColor(202, 202, 202); $pdf->SetTextColor(0, 0, 0);
	...
	$pdf->Cell(100,5,'T.S.U. KARIME CONTRERAS',0,0,'L');$pdf->Cell(120,5,'LCDA. YOSMAR GREHAM',0,0,'L');$pdf->Cell(100,5,'LCDA. ROSIS REQUENA',0,1,'L');
	$pdf->Cell(100,2,'SECRETARIA I',0,0,'L');$pdf->Cell(120,2,'DIRECTOR(A) DE LA DIRECCION DE ADMINISTRACION Y SERVICIOS ',0,0,'L');$pdf->Cell(100,2,'DIRECTORA GENERAL',0,1,'L');
$pdf->Output();
?>  