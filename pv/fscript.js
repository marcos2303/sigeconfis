...
/////////////////////////***************************************///////////////////////////
/////////////////////////***************************************///////////////////////////
function totalReformulacion(rows, insert, update, del) {
	var numreg = document.getElementById("rows");
	numreg.innerHTML="Registros: "+rows;
	//
	var btNuevo=document.getElementById("btNuevo");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
function cargarAprobarAjuste2(form, pagina, param){
	form.method="POST";
	form.action=pagina;
	form.submit();
}
function cargarAprobarAjuste(form, pagina, target, param){
	var codigo=form.registro.value;
	if (codigo=="") msjError(1000);
	else {
		if (target=="SELF") cargarPagina(form, pagina);
		else { pagina=pagina+"?limit=0&accion=VER&registro="+codigo; cargarAprobarAjuste2(form, pagina, param); }
	}
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//// **** FUNCION ELIMINAR ANTEPROYECTO
function eliminarAnteproyecto(form, pagina, foraneo, modulo) {
	var codigo=form.registro.value;
	if (codigo=="") msjError(1000);
	else {
		//	PREGUNTO SI DESEA ELIMINAR
		...
		if (eliminar) {
			if (foraneo) {
				...
				ajax.open("POST", "fphp.php", true);
				ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
				ajax.send("modulo="+modulo+"&accion=ELIMINARANTEPROYECTO&codigo="+codigo);
				ajax.onreadystatechange=function() {
					if (ajax.readyState==4)	{
						var error=ajax.responseText;
						if (error!=0) alert ("¡"+error+"!");
...
//// **** FUNCION ELIMINAR AJUSTE
function eliminarAjuste(form, pagina, foraneo, modulo) {
	var codigo=form.registro.value;
	if (codigo=="") msjError(1000);
	else {
		//	PREGUNTO SI DESEA ELIMINAR
		...
		if (eliminar) {
			if (foraneo) {
				...
				ajax.open("POST", "fphp.php", true);
				ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
				ajax.send("modulo="+modulo+"&accion=ELIMINARAJUSTE&codigo="+codigo);
				ajax.onreadystatechange=function() {
					if (ajax.readyState==4)	{
						var error=ajax.responseText;
						if (error!=0) alert ("¡"+error+"!");
...
function enabledNpresupuesto(form){
	if (form.chknpresupuesto.checked) form.fnpresupuesto.disabled=false; 
	else form.fnpresupuesto.disabled=true; 
}
...
function enabledNrOficio(form) {
	if (form.chkoficio.checked) form.f_oficio.disabled=false; 
	else form.f_oficio.disabled=true; 
}
...
//**************************************  PRESUPUESTO  *************************************************//
//*****************************************************************************************************//
function enabledTajuste(form) {
	if (form.chktajuste.checked) form.ftajuste.disabled=false; 
	else form.ftajuste.disabled=true; 
}
function enabledPFajuste(form){
	if (form.chkfajuste.checked){ form.fdesde.disabled=false;form.fhasta.disabled=false; 
	}else{ form.fdesde.disabled=true;form.fhasta.disabled=true;}
}
function enabledFref(form){
	if (form.chkfref.checked){ form.fdesde.disabled=false;form.fhasta.disabled=false; 
	}else{ form.fdesde.disabled=true;form.fhasta.disabled=true;}
}
function enabledPNajuste(form){
	if (form.chknajuste.checked) form.fnajuste.disabled=false; 
	else form.fnajuste.disabled=true; 
}
function enabledPNref(form){
	if (form.chknref.checked) form.fnref.disabled=false; 
	else form.fnref.disabled=true; 
}


function enabledPEjercicio(form){
	if (form.chkejercicio.checked) form.fejercicio.disabled=false; 
	else {form.fejercicio.disabled=true; form.fejercicio.value='';}
}
//	FUNCION PARA SELECCIONAR UNA PARTIDA DE UNA LISTA Y COLOCARLO EN OTRA VENTANA
function selCodpartida(busqueda,campo){
 var registro=document.getElementById("registro").value; //alert(registro);
	if (campo==1) {
		opener.document.frmentrada.npartida.value=registro;
	} 
	window.close();
}
//*****************************************************************************************************//
...
function selEmpleado(busqueda, campo) {
	...
	if(campo==2){
	    opener.document.frmentrada.nropresupuesto.value = registro;
	}
	window.close();
}
function selEmpleado2(busqueda, campo) {
	var registro=document.getElementById("registro").value; //alert(registro);
	if(campo==1){
	    opener.document.frmentrada.fnropresupuesto.value = registro;
	}
	window.close();
}
//	FUNCION PARA SELECCIONAR UNA PERSONA DE UNA LISTA Y COLOCARLO EN OTRA VENTANA
...
//	FUNCION PARA ELIMINAR REGISTROS DE PARTIDAS CARGADAS EN REFORMULACION
function eliminarPartidaRef(form, pagina) {
	var secuencia = form.registro2.value;
	if (secuencia=="") msjError(1000);
	else {
		//	PREGUNTO SI DESEA ELIMINAR
...
//	FUNCION PARA ELIMINAR REGISTROS DE PARTIDAS CARGADAS EN PRESUPUESTO_DETALLE
function eliminarPartidaEditar(form, pagina) {
	var secuencia = form.registro2.value;
	if (secuencia=="") msjError(1000);
	else {
		//	PREGUNTO SI DESEA ELIMINAR
...
function anularAnteproyecto(form) {
	...
	if (codigo=="") msjError(1000);
		...
		if (anular) {
			...
			ajax.open("POST", "fphp.php", true);
			ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			ajax.send("modulo=ANTEPROYECTO&accion=ANULAR&codigo="+codigo);
			ajax.onreadystatechange=function() {
				if (ajax.readyState==4)	{
					var error=ajax.responseText;
					if (error!=0) alert ("¡"+error+"!");
					else cargarPagina(form, "anteproyecto_listar.php");
				}
			}
		}	
	}
...
function filtroAnteproyecto(form, limit, pagina) {
	...
	if (form.chkorganismo.checked) filtro+=" AND pv_antepresupuesto.CodAnteproyecto LIKE *;"+form.forganismo.value+";*";
	if (form.chknanteproyecto.checked) filtro+=" AND pv_antepresupuesto.CodAnteproyecto LIKE *;"+form.fnanteproyecto.value+";*";
	if (form.chkstatus.checked) filtro+=" AND pv_antepresupuersto.Estado LIKE *"+form.fstatus.value+"*";
...
function numeroBlur(campo) { //alert(campo);
...
function sumarPartida(monto, id) {
	...
	var hora_creada = document.getElementById('horaCreada').value; 
	if(hora_creada==''){
		document.getElementById('horaCreada').value = Date(); //alert('HoraCreada Sumar Partida= '+document.getElementById('horaCreada').value); 
		document.getElementById('permitir').value = 1; //alert('Permitir saumar Partida= '+document.getElementById('permitir').value);
	}else{
		document.getElementById('horaModif').value = Date(); //alert('HoraModif sumar partida= '+ document.getElementById('horaModif').value );
	}
	//var p = Date(); alert(p);    
	var monto_detalle = new Number(setNumero(monto));//alert('MontoDetalle');alert(monto_detalle);
	...
	var valorGuardado = new Number(setNumero(document.getElementById('montovoy').value)); //alert(valorGuardado);
	var valorAnt = new Number(setNumero(document.getElementById('totalAnt').value));//alert('Valor Ant'); alert(valorAnt);
	var total_Antp = new Number (setNumero(document.getElementById('totalAnteproyecto').value));
	...
	if(valorGuardado < monto_detalle ){	
      ...
      var total_partida = monto_partida + calc;
        ...
        document.getElementById("total").value = setNumeroFormato(total_partida,2,'.',',');
		var mTotal = valorAnt + calc;
		document.getElementById("totalAnt").value = setNumeroFormato(mTotal,2,'.',',');
		document.getElementById("totalAnteproyecto").value = setNumeroFormato((total_Antp + calc),2,'.',',');
		//document.getElementById('horaCreada').value = Date(); alert('hora creada Paso 1= '+ document.getElementById('horaCreada').value);
		
	}else{
	  //alert('Paso 2');	
	  var calc = valorGuardado - monto_detalle  ; //alert('calc' + calc);
	  var total_generica = monto_generica - calc;
      var total_partida = monto_partida - calc;
        document.getElementById(generica).value = setNumeroFormato(total_generica,2,'.',',');
        document.getElementById(partida).value = setNumeroFormato(total_partida,2,'.',',');
...
///////////////////////////////////////////////////  ****** SUMAR PARTIDA EN GENERAR PROYECTO
function cambiar(){
	var T_Anteproyecto = new Number(setNumero(document.getElementById('totalAnteproyecto').value.trim()));
	var M_Autorizado = new Number(setNumero(document.getElementById('montoautori').value.trim()));
	if(M_Autorizado > T_Anteproyecto){
      alert('¡NO PUEDE INTRODUCIR UN MONTO SUPERIOR AL MONTO TOTAL DEL ANTEPROYECTO....!');
	  document.getElementById("diferencia").value = '0,00';
	  document.getElementById('montoautori').value = '0,00';
	}else{
		if(M_Autorizado!=''){
			document.getElementById("diferencia").value = setNumeroFormato((T_Anteproyecto - M_Autorizado),2,'.',',');
		}else{
		  alert('¡DEBE CORREGIR EL MONTO AUTORIZADO!');
		  document.getElementById("diferencia").value = '0,00';
		}
	}
}
///////////////////////////////////////////////////  ****** SUMAR PARTIDA GENERADA
///////////////////////////////////////////////////  ****** //////////////////////
function sumarPartidaGen(monto, id) {
...
var id = id.split("|");	
	...
	var monto_generica = new Number(setNumero(document.getElementById(generica).value));
    var monto_partida = new Number(setNumero(document.getElementById(partida).value));
	
	var valorGuardado = new Number(setNumero(document.getElementById('montovoy').value));//alert('Valorguardado=' + valorGuardado);
	var valorAnt = new Number(setNumero(document.getElementById('totalAnt').value));//alert('Valor Ant=' + valorAnt);
	var total_Antp = new Number(setNumero(document.getElementById('totalAnteproyecto').value));//alert('Total_Antp=' + total_Antp);
	var m_Autorizado = new Number(setNumero(document.getElementById('montoautori').value));//alert('MontoAutorizado=' + m_Autorizado);
   
    //alert('MontoDetalle= '+monto_detalle+' Valorguardado=' + valorGuardado+' Valor Ant='+ valorAnt +' MontoAutorizado=' + m_Autorizado);
  
if(valorGuardado < monto_detalle){  //alert('Paso 1');
  var total_generica = monto_generica + monto_detalle - valorGuardado;
  var total_partida = monto_partida + monto_detalle - valorGuardado;
  var valor = valorAnt + monto_detalle; //alert('Valor= '+ valor);
  document.getElementById(generica).value = setNumeroFormato(total_generica,2,'.',',');
  document.getElementById(partida).value =  setNumeroFormato(total_partida,2,'.',',');
  document.getElementById("totalAnt").value = setNumeroFormato(valor,2,'.',','); //alert('ValorAnt= '+ (setNumero(document.getElementById('totalAnt').value))); 
  if((setNumero(document.getElementById('totalAnt').value)) > m_Autorizado){
    var corregir = (setNumero(document.getElementById('totalAnt').value)) - m_Autorizado;
	alert('EL MONTO INTRODUCIDO SUPERA LA DISPONIBILIDAD, CORREGIR= ' + '-'+corregir);
  }
}else{ 
  if(valorGuardado > monto_detalle){ //alert('Paso 2');
	if(monto_detalle==0){
	  var total_generica = monto_generica - valorGuardado;
      var total_partida = monto_partida - valorGuardado;
	  var valor = valorAnt - valorGuardado;
      document.getElementById(generica).value = setNumeroFormato(total_generica,2,'.',',');
      document.getElementById(partida).value =  setNumeroFormato(total_partida,2,'.',',');
      document.getElementById("totalAnt").value = setNumeroFormato(valor,2,'.',','); 
	}else{
	  var calc = valorGuardado - monto_detalle;  
      var total_generica = monto_generica - calc  ;
      var total_partida = monto_partida - calc;
	  var valor = valorAnt - monto_detalle;
      document.getElementById(generica).value = setNumeroFormato(total_generica,2,'.',',');
      document.getElementById(partida).value =  setNumeroFormato(total_partida,2,'.',',');
      document.getElementById("totalAnt").value = setNumeroFormato(valor,2,'.',','); 
      if((setNumero(document.getElementById('totalAnt').value))> m_Autorizado){
    ...
    var total_generica = monto_generica;
    var total_partida = monto_partida;
	var valor = valorAnt;
    document.getElementById(generica).value = setNumeroFormato(total_generica,2,'.',',');
    document.getElementById(partida).value =  setNumeroFormato(total_partida,2,'.',',');
    document.getElementById("totalAnt").value = setNumeroFormato(valor,2,'.',','); 
...
function sumarPartidaEditada(monto, id) {
	...
	var monto_partida = new Number(document.getElementById(partida).value);//alert('MontoPartida');alert(monto_partida);
	var monto_generica = new Number(document.getElementById(generica).value);//alert('MontoGenerica');alert(monto_generica);
	
	// Utilizado para calcular el monto restante
	var valorGuardado = new Number(document.getElementById('montovoy').value);
	var valorMontoautori = new Number(document.getElementById('montoautori').value); //alert('MontoAutorizado');alert(valorMontoautori);
	var valorMontorestante = new Number(document.getElementById('montorestante').value);//alert('MontoRestante');alert(valorMontorestante);
	var valorTotal = new Number(document.getElementById("total").value); //alert('ValorTotal'); alert(valorTotal);
	//var valorAnt = new Number(document.getElementById('totalAnt').value);alert('Valor Ant'); alert(valorAnt);
	
	if (valorGuardado > monto_detalle ){
	  if (monto_detalle < valorMontorestante){// Caso " valorGuardado > monto_detalle " y " monto_detalle < valorMontorestante es igual a cero"
	     var calc = valorGuardado - monto_detalle;
	     var total_generica = monto_generica - calc;
	     var total_partida = monto_partida - calc;
	     document.getElementById(generica).value = total_generica;
		 document.getElementById(partida).value = total_partida;
	     document.getElementById("total").value = valorTotal - calc;
		 document.getElementById("montorestante").value = valorMontorestante + calc;
		 //document.getElementById("totalAnt").value = valorAnt - calc;
	 }else{
	   if (monto_detalle == valorMontorestante){// Caso " valorGuardado > monto_detalle "   
		   var calc = valorGuardado - monto_detalle ;
	       var total_generica = monto_generica + calc;
	       var total_partida = monto_partida + calc;
	       document.getElementById(generica).value = total_generica;
		   document.getElementById(partida).value = total_partida;
	       document.getElementById("total").value = valotTotal + calc;
		   document.getElementById("montorestante").value = valorMontorestante + calc;
		   alert('El monto restante es igual a cero');
	   }else{
		   var calc = valorGuardado - monto_detalle;
		   var total_generica = monto_generica - calc;
	       var total_partida = monto_partida - calc;
	   ...
	   if(valorGuardado < monto_detalle){	
	      var calc = monto_detalle - valorGuardado ;
	      var montoqueda =  valorMontorestante - calc;
		  if(montoqueda < 0){// Verificando que el monto entrante produzca que el monto restante sea menor que cero.
			  alert("El monto introducido debe ser corregido" + montoqueda );
		  }else{
		    if(montoqueda == 0){// Verificando que el monto entrante produzca que el monto restante sea igual a cero.
			  alert('El monto restante es igual a cero');
			  var total_generica = monto_generica + calc;
			  var total_partida = monto_partida + calc;
			  document.getElementById(generica).value = total_generica;
			  document.getElementById(partida).value = total_partida;
			  document.getElementById("total").value = valorTotal + calc;
			  document.getElementById("montorestante").value = montoqueda;
		   }else{
			 var total_generica = monto_generica + calc;
			 var total_partida = monto_partida + calc;
			 document.getElementById(generica).value = total_generica.toFixed(2);
			 document.getElementById(partida).value = total_partida.toFixed(2);
			 var vt = valorTotal + calc;
			 document.getElementById("total").value = vt.toFixed(2); 
			 document.getElementById("montorestante").value = montoqueda;
		  }
		}	  	  
	  }
...
/////////////////////////////////////////// *******  FUNCION QUITAR  
function quitar(registros,ELIMINAR){
  var idFila = document.getElementById("registro2").value;
  var fila = document.getElementById(idFila);
  var trquitar = fila.parentNode;
      trquitar.removeChild(fila);
	   
  //location.href = "anteproyecto_detalle.php?accion=ELIMINAR&registro=fila?"; 
  /*var deleteManager:SQLStatement = new SQLStatement();
	  deleteManager.sqlConnection = conn;
	  deleteManager.text = "DELETE FROM pv_antepresupuestodet WHERE IdAnteProyectoDet= :fila";
	  deleteManager.parameters[":fila"] = id_seleccion;	
