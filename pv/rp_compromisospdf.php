...
function Header(){
  ...
  /*if($Periodo!=""){					   
	...
	$this->Cell(80, 3, 'PROVEEDOR', 1, 0, 'C', 1);
	$this->Cell(35, 3, 'FECHA DOCUMENTO', 1, 0, 'C', 1);
	$this->Cell(35, 3, 'MONTO COMPROMISO', 1, 1, 'C', 1); $this->Ln();
}
//Page footer
function Footer(){
...
$s_con01 = "select 
			 ...
			 where 
			       Estado = 'CO' and CodOrganismo='$Organismo' $filtro2 
			 order by 
			       cod_partida";
	  ...
	  for($j=0;$j<$rows;$j++){
		 ...
		 if($fieldet['partida']!=00){
	        ...
	        $pdf->Row(array($fieldet['cod_partida'],$field['denominacion']));
			 ...
			 $s_distcomp03 = "select 
		                            lgdist.NroDocumento,
									lgdist.CodTipoDocumento,
									lgdist.CodProveedor,
									lgdist.Monto,
									mtp.NomCompleto,
									apobl.FechaDocumento
							    from
							         lg_distribucioncompromisos lgdist
									 inner join mastpersonas mtp on (mtp.CodPersona=lgdist.CodProveedor)
									 inner join ap_obligaciones apobl on (apobl.NroDocumento=lgdist.NroDocumento) 
							  where 
								     lgdist.CodOrganismo = '".$fieldet['Organismo']."' and 
								     lgdist.Estado = 'CO' and 
								     lgdist.cod_partida = '".$fieldet['cod_partida']."' $filtro3
							  order by 
							         apobl.FechaDocumento "; ///echo $s_distcomp03; 
		     ...
		     if($r_distcomp03!=0){
			   for($a=0;$a<$r_distcomp03;$a++){	 
	             ...
	             list($ano, $mes, $dia)=split('[-]',$f_distcomp03['FechaDocumento']);
			     ...
			     $pdf->Cell(25, 5, $f_distcomp03['NroDocumento'], 0, 0, 'L');
			     $pdf->Cell(25, 5, $f_distcomp03['CodTipoDocumento'], 0, 0, 'L');
			     $pdf->Cell(80, 5, $f_distcomp03['NomCompleto'], 0, 0, 'L');
			     $pdf->Cell(25, 5, $FechaDocumento, 0, 0, 'L');
				 $pdf->Cell(25, 5, $monto03, 0, 1, 'L');
			   }
			 }
		 }
     }
