...
//---------------------------------------------------
//echo $Periodo;
global $fechaDesde,$fechaHasta, $CodPresupuesto;
...
function Header(){
	...
	//global $Periodo;
	global $fp_hasta,$fp_desde,$fechaDesde,$fechaHasta;
	//echo $Periodo.'/'.$fp_hasta.'****';
	$this->Image('../imagenes/logos/contraloria.jpg', 10, 10, 10, 10);	
	$this->SetFont('Arial', 'B', 8);
	$this->SetXY(20, 10); $this->Cell(150, 5,utf8_decode( 'Contraloría del Estado Delta Amacuro'), 0, 0, 'L');
	                      $this->Cell(10,5,'Fecha:',0,0,'');$this->Cell(10,5,date('d/m/Y'),0,1,'');
	$this->SetXY(20, 15); $this->Cell(150, 5, utf8_decode('Dirección de Administración y Servicios'), 0, 0, 'L');
  ...
  /*if($Periodo!=""){					   
    ...
    if(($fp_desde!=0)and($fp_hasta!=0)){				   
		if($fp_desde==$fp_hasta){
		    ...
		    $this->Cell(55, 10, utf8_decode('Reporte Compromisos'), 0, 0, 'C'); 
		    $this->Cell(25, 10,  utf8_decode('Período ').''.$fp_hasta, 0, 1, 'R');
		}else{
			$this->SetFont('Arial', 'B', 10);
		    $this->Cell(50, 10, '', 0, 0, 'C');
		    $this->Cell(55, 10, utf8_decode('Reporte Compromisos'), 0, 0, 'C');
		...
		$this->SetFont('Arial', 'B', 10);
		$this->Cell(50, 10, '', 0, 0, 'C');
		$this->Cell(70, 10, utf8_decode('Reporte Compromisos a la Fecha'), 0, 1, 'C');
	}
...
if($CodPresupuesto!=0){
	$spre = "select * from pv_presupuesto where CodPresupuesto='$CodPresupuesto'"; //echo $spre;
    $qpre = mysql_query($spre) or die ($spre.mysql_error());
	$rpre = mysql_num_rows($qpre); 
	if($rpre!=0) $fpre = mysql_fetch_array($qpre);	
	$Anio = "and Anio='$fpre[EjercicioPpto]'";
}
/// ---------------------------------------------------------------------------------------
///   Consulta tabla lg_distribucioncompromisos para obtener las partidas involucradas
...
$s_con01 = "select 
			 ...
			 where 
			       Estado='CO' and CodOrganismo='$Organismo' $Anio $filtro2 
			 order by 
			       cod_partida"; //echo $s_con01;
...
$s_distcomp03 = "(SELECT
    ...
    dc.CodTipoDocumento,
    dc.NroDocumento,
    oc.FechaAprobacion AS Fecha,
    oc.NomProveedor
FROM
    lg_distribucioncompromisos dc
    INNER JOIN pv_partida p ON (p.cod_partida = dc.cod_partida)
    INNER JOIN lg_ordencompra oc ON (oc.CodOrganismo = dc.CodOrganismo AND
                                     oc.Anio = dc.Anio AND
                                     oc.NroOrden = dc.NroDocumento)
WHERE
    dc.Origen = 'OC' and 
	dc.cod_partida='".$fieldet['cod_partida']."' and 
	oc.FechaAprobacion>='$fechaDesde' and 
	oc.FechaAprobacion<='$fechaHasta')
...
WHERE
    dc.Origen = 'OS' and 
	dc.cod_partida='".$fieldet['cod_partida']."' and 
	os.FechaAprobacion>='$fechaDesde' and  
	os.FechaAprobacion<='$fechaHasta')
...
FROM
    ...
    INNER JOIN mastpersonas ps ON (ps.CodPersona = dc.CodProveedor)
WHERE
    dc.Origen = 'OB' and 
	dc.cod_partida='".$fieldet['cod_partida']."' and 
	o.FechaAprobado>='$fechaDesde' and 
	o.FechaAprobado<='$fechaHasta')
...
FROM
    lg_distribucioncompromisos dc
    INNER JOIN pv_partida p ON (p.cod_partida = dc.cod_partida)
    INNER JOIN ap_bancotransaccion bt ON (bt.CodOrganismo = dc.CodOrganismo AND
                                          bt.CodTipoDocumento = dc.CodTipoDocumento AND
                                          bt.CodigoReferenciaBanco = dc.NroDocumento AND
                                          bt.CodProveedor = dc.CodProveedor)
    INNER JOIN mastpersonas ps ON (ps.CodPersona = dc.CodProveedor)
WHERE
    dc.Origen = 'TB' and 
	dc.cod_partida='".$fieldet['cod_partida']."' and 
	bt.FechaTransaccion>='$fechaDesde' and 
	bt.FechaTransaccion<='$fechaHasta')
