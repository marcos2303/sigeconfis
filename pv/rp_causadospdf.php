...
$s_distcomp03 = "(SELECT
					...
					FROM
						siaceda.ap_distribucionobligacion do
						INNER JOIN siaceda.pv_partida p ON (p.cod_partida = do.cod_partida)
						INNER JOIN siaceda.ap_obligaciones o ON (o.CodProveedor = do.CodProveedor AND
														 o.CodTipoDocumento = do.CodTipoDocumento AND
														 o.NroDocumento = do.NroDocumento)
						INNER JOIN siaceda.mastpersonas ps ON (ps.CodPersona = do.CodProveedor)
					WHERE
					...
					FROM
						siaceda.ap_distribucionobligacion do
						INNER JOIN siaceda.pv_partida p ON (p.cod_partida = do.cod_partida)
						INNER JOIN siaceda.ap_bancotransaccion bt ON (bt.CodOrganismo = do.CodOrganismo AND
