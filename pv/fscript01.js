...
/// ***********************************************************************************
/// --------------------------   ANULAR AJUSTE PRESUPUESTARIO ----------------------///
function anularAjuste(form) {
	var codigo=form.registro.value;
	if (codigo=="") msjError(1000);
	else {
		//	PREGUNTO SI DESEA ANULAR
		var anular=confirm("¡Esta seguro de anular este Ajuste?");
		if (anular) {
			//	CREO UN OBJETO AJAX PARA VERIFICAR QUE EL NUEVO REGISTRO NO EXISTA EN LA BASE DE DATOS
			var ajax=nuevoAjax();
			ajax.open("POST", "fphp.php", true);
			ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			ajax.send("modulo=AJUSTE_PRESUPUESTARIO&accion=ANULAR&codigo="+codigo);
			ajax.onreadystatechange=function() {
				if (ajax.readyState==4)	{
					var error=ajax.responseText;
					if (error!=0) alert ("¡"+error+"!");
					else cargarPagina(form, "presupuesto_ajustelistar.php");
				}
			}
		}	
	}
...
/// ***********************************************************************************
/// -------------------------- PROCESOS DEL DOCTOR PRESUPUESTARIO
function ProcesarCambios(form, valor){ alert(valor);
  var codigo=form.registro.value; alert(codigo);
  var CodPresupuesto = document.getElementById("cod_presupuesto").value;
  var Organismo = document.getElementById("fOrganismo").value; 
  
  if(codigo=="") msjError(1000);
  else{ 
    alert("accion=DOCTORPRESUPUESTARIO&codigo="+codigo+"&valor="+valor+"&CodPresupuesto="+CodPresupuesto+"&Organismo="+Organismo);
    var ajax=nuevoAjax();
		ajax.open("POST", "fphp_ajax.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("accion=DOCTORPRESUPUESTARIO&codigo="+codigo+"&valor="+valor+"&CodPresupuesto="+CodPresupuesto+"&Organismo="+Organismo);
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4)	{
				var error=ajax.responseText;
				if (error!=0) alert ("¡"+error+"!");
				else cargarPagina(form, "proceso_doctorpresupuestario.php");
			}
		}
  }
  return false;
