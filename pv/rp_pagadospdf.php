...
function Header(){
  ...
  /*if($Periodo!=""){					   
    ...
    if(($fp_desde!=0)and($fp_hasta!=0)){				   
		...
		$this->Cell(55, 10, utf8_decode('Reporte Pagos'), 0, 0, 'C');
		$this->Cell(20, 10, 'Desde '.$fp_desde, 0, 0, 'R'); 
		$this->Cell(25, 10, 'Hasta '.$fp_hasta, 0, 1, 'R');
	}else{
	   $this->SetFont('Arial', 'B', 10);
	   $this->Cell(50, 10, '', 0, 0, 'C');
	   $this->Cell(70, 10, utf8_decode('Reporte Pagos a la Fecha'), 0, 1, 'C');
	}
...
$s_con01 = "select 
			 ...
			 where 
			       Estado='PA' and CodOrganismo<>'' $filtro2 
			 order by 
			       cod_partida";
...
FROM
    siaceda.ap_ordenpagodistribucion dp
    INNER JOIN siaceda.pv_partida p ON (p.cod_partida = dp.cod_partida)
    INNER JOIN siaceda.ap_pagos pa ON (pa.CodProveedor = dp.CodProveedor  AND
                                       pa.NroOrden = dp.NroOrden)
    INNER JOIN siaceda.mastpersonas ps ON (ps.CodPersona = dp.CodProveedor)
WHERE
...
FROM
    siaceda.ap_ordenpagodistribucion dp
    INNER JOIN siaceda.pv_partida p ON (p.cod_partida = dp.cod_partida)
    INNER JOIN siaceda.ap_bancotransaccion bt ON (bt.CodOrganismo = dp.CodOrganismo AND
