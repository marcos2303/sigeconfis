...
extract($_POST);
extract($_GET);
include ("fphp_ajax.php");
include ("fphp_ajax1.php");
...
function getSector($sector, $opt) {
	...
	switch ($opt) {
		case 0:
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field[0]==$sector) echo "<option value='".$field[0]."' selected>".htmlentities($field[1]); 
				else echo "<option value='".$field[0]."'>".$field[0]."-".htmlentities($field[1])."</option>";
			}
			break;
		case 1:
...
function getPrograma($programa, $sector, $opt) {
	connect();
	switch ($opt) {
		case 0:
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field[0]==$programa) echo "<option value='".$field[0]."' selected>".htmlentities($field[2]); 
...
function getSubprograma($subprograma, $programa, $opt) {
	connect();
	switch ($opt) {
		case 0:
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field[0]==$subprograma) echo "<option value='".$field[0]."' selected>".htmlentities($field[2])."</option>"; 
				else echo "<option value='".$field[0]."'>".$field[1]."-".htmlentities($field[2])."</option>";
			}
			break;
		case 1:
...
function getProyecto($proyecto, $subprograma, $opt) {
	connect();
	switch ($opt) {
		case 0:
			$sql="SELECT id_proyecto,cod_proyecto,descp_proyecto,id_sub FROM pv_proyecto1 WHERE id_sub='".$subprograma."'";
			$query=mysql_query($sql) or die ($sql.mysql_error());
			$rows=mysql_num_rows($query);
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field[3]==$proyecto) echo "<option value='".$field[0]."' selected>".htmlentities($field[2])."</option>"; 
				else echo "<option value='".$field[0]."'>".$field[1]."-".htmlentities($field[2])."</option>";
			}
			break;
		case 1:
...
//	FUNCION PARA CARGAR PROYECTO DE PRESUPUESTO EN UN SELECT
function getActividad($actividad, $proyecto, $opt) {
	connect();
	switch ($opt) {
		case 0:
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field[0]==$actividad) echo "<option value='".$field[0]."' selected>".htmlentities($field[2])."</option>"; 
				else echo "<option value='".$field[0]."'>".$field[1]."-".htmlentities($field[2])."</option>";
			}
			break;
		case 1:
...
function getOrganismos($organismo, $opt, $Organismo) {
	connect();
...
function getOrganismos2($organismo, $opt) {
	connect();
	if ($opt==3 && $_SESSION["USUARIO_ACTUAL"]==$_SESSION["SUPER_USUARIO"]) $opt=0;
	switch ($opt) {
		case 0:
			...
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if($field[1]==$_SESSION[ORGANISMO_ACTUAL]){
				   echo "<option value='".$field[0]."'>".htmlentities($field[1])."</option>";
				}else{
				  if ($field[0]==$organismo) echo "<option value='".$field[0]."' selected>".htmlentities($field[1])."</option>"; 
				  else echo "<option value='".$field[0]."'>".htmlentities($field[1])."</option>";
...
//////////////////////////////////////**************************************////////////////////////////////////////
//	FUNCION PARA CARGAR INFORMACION DE QUIENES HAN PREPARADO ANTEPROYECTOS EN UN SELECT
function getPreparado($preparado, $organismo, $opt) {
	connect();
	if ($opt==3 && $_SESSION["USUARIO_ACTUAL"]==$_SESSION["SUPER_USUARIO"]) $opt=0;
	switch ($opt) {
		case 0:
			$sql="SELECT CodAnteproyecto,PreparadoPor FROM pv_antepresupuesto WHERE Organismo='".$organismo."' ORDER BY CodAnteproyecto";
			$query=mysql_query($sql) or die ($sql.mysql_error());
			$rows=mysql_num_rows($query);                 
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if ($field[0]==$preparado) echo "<option value='".$field[0]."' selected>".htmlentities($field[12])."</option>"; 
				else echo "<option value='".$field['CodAnteproyecto']."'>".htmlentities($field['PreparadoPor'])."</option>";
			}
			break;
		case 1:
			$sql="SELECT CodAnteproyecto,PreparadoPor FROM pv_antepresupuesto WHERE CodAnteproyecto='$preparado'";
			$query=mysql_query($sql) or die ($sql.mysql_error());
			$rows=mysql_num_rows($query);
			if ($rows!=0) {
				$field=mysql_fetch_array($query);
				echo "<option value='".$field['CodAnteproyecto']."'>".htmlentities($field['PreparadoPor'])."</option>";
			}
			break;
		case 3:
			$sql="SELECT DISTINCT PreparadoPor FROM pv_antepresupuesto WHERE 1";
			$query=mysql_query($sql) or die ($sql.mysql_error());
			$rows=mysql_num_rows($query);
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
...
function getGCargo($grado, $opt) {
	connect();
	$tgrado[0]="01"; $vgrado[0]="01";
	$tgrado[1]="02"; $vgrado[1]="02";
	$tgrado[2]="03"; $vgrado[2]="03";
	$tgrado[3]="04"; $vgrado[3]="04";
	$tgrado[4]="05"; $vgrado[4]="05";
	$tgrado[5]="06"; $vgrado[5]="06";
	$tgrado[6]="07"; $vgrado[6]="07";
	$tgrado[7]="08"; $vgrado[7]="08";
	$tgrado[8]="09"; $vgrado[8]="09";
	$tgrado[9]="10"; $vgrado[9]="10";
	$tgrado[10]="99"; $vgrado[10]="99";
	switch ($opt) {
		case 0:
			for ($i=0; $i<11; $i++) {
				if ($grado==$vgrado[$i]) echo "<option value='".$vgrado[$i]."' selected>".$tgrado[$i]."</option>";
				else echo "<option value='".$vgrado[$i]."'>".$tgrado[$i]."</option>";
			}
			break;
		case 1:
			for ($i=0; $i<10; $i++) {
...
//	FUNCION PARA CARGAR LOS PREPARADOS EN UN SELECT PARA ANTEPROYECTO FILTRADO
function getPreparadoPor($fpreparado, $opt) {
	connect();
	switch ($opt) {
		case 0:
			$sql="SELECT DISTINCT PreparadoPor FROM pv_antepresupuesto";
			$query=mysql_query($sql) or die ($sql.mysql_error());
			$rows=mysql_num_rows($query);
			for ($i=0; $i<$rows; $i++) {
				$field=mysql_fetch_array($query);
				if($fpreparado==$field[0]){
				  echo "<option value='".$field['PreparadoPor']."' selected>".htmlentities($field[0])."</option>";
				}else{
				  echo "<option value='".$field['PreparadoPor']."'>".htmlentities($field[0])."</option>";
				}
			}
			break;
		/*case 1:
...
//	FUNCION PARA CARGAR LOS TIPOS DE STATUS EN UN SELECT PARA ESTADO ANTEPROYECTO
function getStatusAnteproyecto($status, $opt) {
	connect();
	$tstatus[0]="Preparado"; $vstatus[0]="PE"; 
	$tstatus[1]="Revisado";  $vstatus[1]="RV";
	$tstatus[2]="Aprobado";  $vstatus[2]="AP";
	$tstatus[3]="Generado";  $vstatus[3]="GE";
	$tstatus[4]="Anulado";   $vstatus[4]="AN";
	switch ($opt) {
		case 0:
			for ($i=0; $i<5; $i++) {
				if ($status==$vstatus[$i]) echo "<option value='".$vstatus[$i]."' selected>".$tstatus[$i]."</option>";
...
//	FUNCION PARA CARGAR LOS TIPOS DE STATUS EN UN SELECT PARA ESTADO ANTEPROYECTO
function getEstadoAntp($status, $opt) {
	connect();
	$tstatus[0]="Preparado"; $vstatus[0]="PE";
	$tstatus[1]="Revisado"; $vstatus[1]="RV";
	$tstatus[2]="Aprobado"; $vstatus[2]="AP";
...
//	FUNCION PARA CARGAR LOS TIPOS DE STATUS EN UN SELECT PARA ESTADO AJUSTE
function getEstadoAjuste($status, $opt) {
	connect();
...
//	FUNCION PARA CARGAR LOS TIPOS DE STATUS EN UN SELECT PARA ESTADO ANTEPROYECTO
function getTipoAjuste($status, $opt) {
	connect();
	$tstatus[0]="Incremento"; $vstatus[0]="IN";
	$tstatus[1]="Disminuci&oacute;n"; $vstatus[1]="DI";
	switch ($opt) {
		case 0:
			for ($i=0; $i<5; $i++) {
				if ($status==$vstatus[$i]) echo "<option value='".$vstatus[$i]."' selected>".$tstatus[$i]."</option>";
...
function getTEvaluaciones2($valor, $opt) {
	connect();
	$tvalor[0]="90º"; $vvalor[0]="90";
	$tvalor[1]="180º"; $vvalor[1]="180";
	$tvalor[2]="360º"; $vvalor[2]="360";
	$total=3;
	switch ($opt) {
		case 0:
			for ($i=0; $i<$total; $i++) {
...
//// ------------------------------------------------------------------------------
function getAjuste2($valor,$opt){
   switch ($opt) {
		case 0:
			if($valor=DI){
			  echo"<option value='RP'>Rebaja Presupuestaria</option>
	               <option value='TI'>Traslados Internos</option>";
			}else{
			  echo"<option value='CA'>Credito Adicional</option>
	               <option value='EE'>EEEEEEEEEEEEEEEEE</option>";
			}
			break;
		case 1:
			$sql="SELECT Plantilla, Descripcion FROM rh_evaluacionfactoresplantilla WHERE Plantilla='$valor'";
  ...
  switch ($opt){
	  case 0:
		 ...
		 for($i=0;$i<$row;$i++){
		   ...
		   if($field['CodLibroCont']==$fLibro)
		     echo"<option value='".$field['CodLibroCont']."' selected>".$field['Descripcion']."<option/>";
		   else
		     echo"<option value='".$field['CodLibroCont']."'>".$field['Descripcion']."<option/>";	 	 
		 }
	     break;
  }
}
//// ------------------------------------------------------------------------------
//// **** ELIMINAR y ANULAR PARTIDA DE REFORMULACION
if($accion=="EliminarPartidaReformulacion"){
	connect();
	$sql="DELETE FROM 
	                  pv_reformulacionpptodet 
	            WHERE 
				      cod_partida='$valor' AND
				      Organismo='$Organismo' AND
					  CodPresupuesto='$CodPresupuesto' AND
					  CodRef='$CodRef'"; //echo $sql;
	$query=mysql_query($sql) or die ($sql.mysql_error());
}
if($accion=="EliminarPartReforCanc"){
	connect();
	$sql="DELETE FROM 
	                  pv_reformulacionpptodet 
	            WHERE 
...
if($accion=="anularReformulacion"){
  connect();
  list($Organismo, $CodPresupuesto, $CodRef) = split('[-]',$codigo);
  $sql = "select 
                * 
		    from  
  ...
  if($field['Estado']=='AN'){
	  echo "El registro se encuentra anulado";
  }else{
    $supdate = "update 
                    pv_reformulacionppto 
				 set 
...
if($accion=="eliminarReformulacion"){
  connect();
  list($Organismo, $CodPresupuesto, $CodRef) = split('[-]',$codigo);
  $sql = "select Estado from pv_reformulacionppto where Organismo='$Organismo' and CodPresupuesto='$CodPresupuesto' and CodRef='CodRef'";
  $qry = mysql_query($sql) or die ($sql.mysql_error());
  $field = mysql_fetch_array($qry);
  
  if($field['Estado']=='AP'){
     echo"No puede ser eliminado por estar ya en estado Aprobado";  
  }else{
     $sdelete = "delete from pv_reformulacionppto where Organismo='$Organismo' and CodPresupuesto='$CodPresupuesto' and CodRef='$CodRef'";
     $qdelete = mysql_query($sdelete) or die ($sdelete.mysql_error());
	 
	 $sdelete = "delete from pv_reformulacionpptodet where Organismo='$Organismo' and CodPresupuesto='$CodPresupuesto' and CodRef='$CodRef'";
     $qdelete = mysql_query($sdelete) or die ($sdelete.mysql_error());
  }
...
//// ------------------------------------------------------------------------------
function getEstadoReformulacion($status, $opt) {
	connect();
	$tstatus[0]="Preparado"; $vstatus[0]="PR";
	$tstatus[1]="Aprobado"; $vstatus[1]="AP";
	$tstatus[2]="Anulado"; $vstatus[2]="AN";
	switch ($opt) {
		case 0:
