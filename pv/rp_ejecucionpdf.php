...
//// --------------------------------------------------------------------------------------------------
 ...
 if(($fieldet['partida']!=00) and (($cont2==0) or ($gCapturada!=$fieldet['generica']) or ($pCapturada2!=$fieldet['partida']))){
	...
	if($rows2!=0){
	  ...
	  $pdf->Row(array($codigo_partida,$fpar2['denominacion'],$montoAprobado1,$montoAjustado1,$montoCompromiso1,$montoCausado1,$montoPagado1,$montoDisponible1));
   }
  }
//// --------------------------------------------------------------------------------------------------
...
//// --------------------------------------------------------------------------------------------------
 if($fieldet['partida']!=00){
	 ...
	 $pdf->SetWidths(array(18, 100,25,25,25,25,25,25));
	 $pdf->SetAligns(array('C','L','R','R','R','R','R','R'));
	 $pdf->Row(array($fieldet['cod_partida'],$field['denominacion'],$montoAprobado2,$montoAjustado2,$montoCompromiso2,$montoCausado2, $montoPagado2, $montoDisponible2));
 }
}
	///// *** Mostrar *** /////
	...
	$pdf->Row(array('' ,'Total:',$totalMontoAprobado,$totalMontoAjustado,$totalMontoCompromiso,$totalMontoCausado,$totalMontoPagado,$totalDisponible));
	/////
//---------------------------------------------------*/
/*$pdf->SetDrawColor(0, 0, 0); $pdf->SetFillColor(202, 202, 202); $pdf->SetTextColor(0, 0, 0);
