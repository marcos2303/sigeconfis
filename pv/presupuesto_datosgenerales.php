...
if(mysql_num_rows($qry)!=0){
  ...
  if(($fielDet['generica']!=00) and (($cont2==0) or ($gCapturada!=$fielDet['generica']) or ($pCapturada2!=$fielDet['partida']))){
   ...
   if($fielDet['partida']!=00){
	...
	$total = $monto + $total;
	$totalAjustado = $totalAjustado + $fielDet['MontoAjustado']; // Monto Ajustado
	$totalCompromiso = $totalCompromiso + $fielDet['MontoCompromiso']; // Monto Compromiso
	$totalCausado = $totalCausado + $fielDet['MontoCausado']; // Monto Causado
	$totalPagado = $totalPagado + $fielDet['MontoPagado']; // Monto Pagado
	$totalDisponible = $totalDisponible + $totalAjustado - $totalCompromiso; // Monto Disponible
    ...
    echo "<tr class='trListaBody' onclick='mClk(this,\"registro2\");'>
	   ...
	   echo"<input type='text' style='text-align:right' id='totalAnt' name='totalAnt' size='13' value='$totalDisponible' readonly/>
