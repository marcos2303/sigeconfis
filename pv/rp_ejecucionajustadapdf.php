...
function Header(){
	...
	$this->SetXY(20, 10); $this->Cell(200, 5,utf8_decode( 'Contraloría del Estado Delta Amacuro'), 0, 0, 'L');
  ...
  /// CONDICION PARA CUANDO LOS PERIODOS SON DISTINTOS -------------------- /////// 
    ...
    $this->SetFont('Arial', 'B', 10);
	$this->Cell(105, 10, '', 0, 0, 'C');
	$this->Cell(65, 10, utf8_decode('Ejecución Presupuestaria 2012'), 0, 0, 'C');
	...
	$this->Cell(100, 3, 'DENOMINACION', 1, 0, 'C', 1);
	$this->Cell(23, 3, 'T.PRESUPUESTADO', 1, 0, 'C', 1);
	$this->Cell(23, 3, 'INCREMENTOS', 1, 0, 'C', 1);
	$this->Cell(23, 3, 'DISMINUCIONES', 1, 0, 'C', 1);
	$this->Cell(23, 3, 'T. AJUSTADO', 1, 0, 'C', 1);
	$this->Cell(23, 3, 'T. PAGADO', 1, 0, 'C', 1);
	$this->Cell(23, 3, 'T. DISPONIBLE', 1, 1, 'C', 1);
...
for($i=0; $i<$rows ; $i++){
 ...
 if(($fieldet['partida']!=00) and (($cont1==0) or ($pCapturada!=$fieldet['partida']))){
  ...
  if($rwPar!=0){
   ...
   if($r_ejecucion01!=0) $f_ejecucion01 = mysql_fetch_array($q_ejecucion01);
	...
	$pdf->SetWidths(array(18, 100,23,23,23,23,23,23));
	$pdf->SetAligns(array('C','L','R','R','R','R','R','R'));
	$pdf->Row(array($codigo_partida,$fpar['denominacion'],$montoDP,$montoIncrementoTotal1,$montoDisminucionTotal1,$montoPptoAjustado1,$montoPagadoTotal1,$montoDisponPresupuestaria1,$montoDisponFinanciera1));
  }
 }
 //// ----------------------------------------------------------------------------------------------- */
 ...
 if(($fieldet['partida']!=00) and (($cont2==0) or ($gCapturada!=$fieldet['generica']) or ($pCapturada2!=$fieldet['partida']))){
	...
	if($rows2!=0){
	  ...
	  $pdf->SetWidths(array(18, 100,23,23,23,23,23,23));
	  $pdf->SetAligns(array('C','L','R','R','R','R','R','R'));
	  $pdf->Row(array($codigo_partida,$fpar2['denominacion'],$montoDP2,$montoIncrementoTotal2,$montoDisminucionTotal2,$montoPptoAjustado2,$montoPagadoTotal2,$montoDisponPresupuestaria2, $montoDisponFinanciera2,$montoVariacion2));
   }
  }
 //// ----------------------------------------------------------------------------------------------- */
 ...
 if($fieldet['partida']!=00){
	 ...
	 $pdf->SetWidths(array(18, 100,23,23,23,23,23,23));
	 $pdf->SetAligns(array('C','L','R','R','R','R','R','R'));
	 $pdf->Row(array($fieldet['cod_partida'],$field['denominacion'],$montoDP3,$montoIncrementoTotal3,$montoDisminucionTotal3,$montoPptoAjustado3,$montoPagadoTotal3,$montoDisponPresupuestaria3,$montoDisponFinanciera3,$montoVariacion3));
 }
}
	///// *** Mostrar *** /////
...
$sqlDet="SELECT cod_partida, MontoAprobado, partida, 
                generica, especifica, MontoPagado, subespecifica, 
				tipocuenta, CodPresupuesto, Organismo, MontoAjustado
		   FROM 
		        pv_presupuestodet 
		  WHERE 
		        Organismo<>'' and 
...
for($i=0; $i<$rows ; $i++){
 ...
 if(($fieldet['partida']!=00) and (($cont1==0) or ($pCapturada!=$fieldet['partida']))){
  ...
  if($rwPar!=0){
   ...
   $sqldet="SELECT MontoAprobado, cod_partida, CodPresupuesto,
				   Organismo, MontoPagado, MontoAjustado 
		      FROM pv_presupuestodet 
		     WHERE partida='".$fpar['partida1']."' AND 
			       tipocuenta='".$fpar['cod_tipocuenta']."' AND 
				   CodPresupuesto='".$fieldet['CodPresupuesto']."' and 
   ...
   for($a=0; $a<$rwdet; $a++){
	...
	$montoPagado+= $fdet['MontoPagado'];
	$montoAjustado+= $fdet['MontoAjustado'];
	...
	$sql_a = "select 
			   ...
			   from 
					...
					inner join pv_ajustepresupuestario b on (b.CodPresupuesto = a.CodPresupuesto)and(b.Organismo=a.Organismo)and(b.CodAjuste=a.CodAjuste)and(b.Estado='AP')
		      where 
			        a.CodPresupuesto='".$fdet['CodPresupuesto']."' and 
					a.Organismo='".$fdet['Organismo']."' and 
					a.cod_partida='".$fdet['cod_partida']."' and 
					a.Estado='AP'"; //echo $sql_a;
	...
	if($row_a!=0){
      for($s=0;$s<$row_a;$s++){
		 ...
		 if($field_a['TipoAjuste']=='IN'){
		    $montoCal4 = $montoCal4 + $field_a['MontoAjuste']; //echo $field_a['MontoAjuste'];  // Calculando el Incremento
		 }else{
		    $montoCal5 = $montoCal5 + $field_a['MontoAjuste']; // Calculando la Disminución
		 }
	  }	
	}
   }
	   
	$montoPagado = number_format($montoPagado,2,',','.'); //// Monto Pagado
	$montoAjustado = number_format($montoAjustado,2,',','.'); //// Monto Ajustado
	$montoDisponible = $montoAjustado -  $montoPagado;  
	$montoDisponible01 =  number_format($montoDisponible,2,',','.'); //// Monto Disponible
	
	$monto_presupuestario = number_format($m_pdet,2,',','.'); /// Monto Presupuestario Inicial y Financiamiento Inicial
	   
	$m_Incremento = number_format($montoCal4,2,',','.'); if($m_Incremento=="")$m_Incremento='0.00';
	$m_Disminucion = number_format($montoCal5,2,',','.');if($m_Disminucion=="")$m_Disminucion='0.00';
	...
	$pdf->SetWidths(array(18, 100,23,23,23,23,23,23));
	$pdf->SetAligns(array('C','L','R','R','R','R','R','R'));
	$pdf->Row(array($codigo_partida,$fpar['denominacion'],$monto_presupuestario,$m_Incremento,$m_Disminucion,$montoAjustado,$montoPagado,$montoDisponible01));
  }
 }
 //// ********************************************************************** 
 ////             Capturando Partida Tipo "T" 301-01-00-00
 ...
 if(($fieldet['partida']!=00) and (($cont2==0) or ($gCapturada!=$fieldet['generica']) or ($pCapturada2!=$fieldet['partida']))){
	...
	if($rows2!=0){
	  $fpar2=mysql_fetch_array($qry2);
	  $montoG=0; $cont2=0; $montoConsulta02=0;
	  $sqldet2="SELECT MontoAprobado, cod_partida,CodPresupuesto,Organismo,MontoPagado,MontoAjustado
			      FROM pv_presupuestodet 
			     WHERE partida='".$fpar2['partida1']."' AND 
				       generica='".$fpar2['generica']."' AND 
					   tipocuenta='".$fpar2['cod_tipocuenta']."' AND 
	  ...
	  for($b=0; $b<$rwdet2; $b++){	  
	   $fdet2=mysql_fetch_array($qrydet2);
	   $cont2 = $cont2 + 1;
	   $monto2+= $fdet2['MontoAprobado'];
	   $montoPagado2+= $fdet2['MontoPagado'];
	   $montoAjustado2+= $fdet2['MontoAjustado'];
		...
		$sql_a = "select 
			   ...
			   from 
					...
					inner join pv_ajustepresupuestario b on (b.CodPresupuesto = a.CodPresupuesto)and(b.Organismo=a.Organismo)and(b.CodAjuste=a.CodAjuste)and(b.Estado='AP')
		      where 
			        a.CodPresupuesto='".$fdet2['CodPresupuesto']."' and 
					a.Organismo='".$fdet2['Organismo']."' and 
					a.cod_partida='".$fdet2['cod_partida']."' and 
					a.Estado='AP'"; //echo $sql_a;
    $qry_a = mysql_query($sql_a) or die ($sql_a.mysql_error());
	$row_a = mysql_num_rows($qry_a);
	if($row_a!=0){
      for($y=0;$y<$row_a;$y++){
	     $field_a = mysql_fetch_array($qry_a);
		 if($field_a['TipoAjuste']=='IN'){
		    $montoIncremento2+=$field_a['MontoAjuste']; // Calculando el Incremento
		 }else{
		    $montoDisminucion2+=$field_a['MontoAjuste']; // Calculando la Disminución
		 }
	  }	
	}		
 } 
 
    $montoDisponible2 = $montoAjustado2 - $montoPagado2;
	
	  $montoIncremento2 = number_format($montoIncremento2,2,',','.'); if($montoIncremento2=="")$montoIncremento2='0.00';/// monto incremento
	  $montoDisminucion2 = number_format($montoDisminucion2,2,',','.');if($montoDisminucion2=="")$montoDisminucion2='0.00';/// monto disminucion
	  $montoAjustado2 = number_format($montoAjustado2,2,',','.'); /// monto ajustado
	  $montoPagado2 = number_format($montoPagado2,2,',','.'); /// monto pagado
	  $montoDisponible2 = number_format($montoDisponible2,2,',','.'); /// monto disponible
	  
	  $monto2=number_format($monto2,2,',','.'); /// monto partida
	  $codigo_partida = $fpar2['cod_partida'];
	  $gCapturada = $fpar2['generica'];
	  $pCapturada2 = $fpar2['partida1'];
	  ...
	  $pdf->SetWidths(array(18, 100,23,23,23,23,23,23));
	  $pdf->SetAligns(array('C','L','R','R','R','R','R','R'));
	  $pdf->Row(array($codigo_partida,$fpar2['denominacion'],$monto2, $montoIncremento2, $montoDisminucion2, $montoAjustado2, $montoPagado2, $montoDisponible2));
   }
  }
 //// ********************************************************************** 
 ////       Capturando Partida Tipo "D" 301-01-01-00 ó 301-01-01-01
 //// ********************************************************************** 
 $montoCalculadoIncremento=''; 
 $montoCalculadoDisminucion=''; 
 $montoCalculadoPagado = '';
 $montoPagado03 = '';
 $montoAjustado03 = '';
 $montoDisponible03 = ''; 
 ...
 if($fieldet['partida']!=00){
	 ...
	 $monto = $fieldet['MontoAprobado']; //// Monto Partida
	 $montoT=$montoT + $monto; 
	 $montoTotal=number_format($montoT,2,',','.');
	 $monto=number_format($monto,2,',','.');
	 
	 
	 $montoPagado03 = $fieldet['MontoPagado']; /// Monto Pagado
	 $montoPagadoTotal+=$montoPagado03;
	 $montoAjustado03 = $fieldet['MontoAjustado']; /// Monto Ajustado
	 $montoAjustadoTotal+= $montoAjustado03;
	 
	 $montoDisponible03 = $montoAjustado03 - $montoPagado03; /// Monto Dispopnible
	 $montoDisponibleTotal+=$montoDisponible03;
	 
	 $montoPagado03=number_format($montoPagado03,2,',','.'); /// monto pagado
	 $montoAjustado03=number_format($montoAjustado03,2,',','.'); /// monto ajustado
	 $montoDisponible03=number_format($montoDisponible03,2,',','.'); // monto disponible
	 /// ************************************************************************* 
	 ///                         Monto Incrementado y Disminuido
	 /// *************************************************************************  
	   $sql_a = "select 
	...
	if($row_a!=0){
      for($x=0;$x<$row_a;$x++){
	     $field_a = mysql_fetch_array($qry_a);
		 if($field_a['TipoAjuste']=='IN'){
		    $montoCalculadoIncremento = $montoCalculadoIncremento + $field_a['MontoAjuste']; // Calculando el Incremento
		 }else{
		    $montoCalculadoDisminucion = $montoCalculadoDisminucion + $field_a['MontoAjuste']; // Calculando la Disminución
		 }
	  }	
	}		
	   
	$montoIncrementoTotal+= $montoCalculadoIncremento; /// Monto Incremento	
    $montoDisminucionTotal+= $montoCalculadoDisminucion;/// Monto Disminucion	
	
	$montoCalculadoIncremento = number_format($montoCalculadoIncremento,2,',','.'); if($montoCalculadoIncremento=="")$montoCalculadoIncremento='0.00';  /// Incremento  
	$montoCalculadoDisminucion = number_format($montoCalculadoDisminucion,2,',','.');if($montoCalculadoDisminucion=="")$montoCalculadoDisminucion='0.00';   /// Disminucion 	
		 
	 $pdf->SetDrawColor(0, 0, 0); $pdf->SetFillColor(255, 255, 255); $pdf->SetTextColor(0, 0, 0);
	 $pdf->SetFont('Arial', '', 6.5);
	 $pdf->SetWidths(array(18, 100,23,23,23,23,23,23));
	 $pdf->SetAligns(array('C','L','R','R','R','R','R','R'));
	 $pdf->Row(array($fieldet['cod_partida'],$field['denominacion'],$monto,$montoCalculadoIncremento,$montoCalculadoDisminucion,$montoAjustado03,$montoPagado03,$montoDisponible03));
 }
}
	/// ****** ------------------------------------------------------------------- ****** ///    
	
	$montoIncrementoTotal = number_format($montoIncrementoTotal,2,',','.');  // Calculando el Total Incremento
	$montoDisminucionTotal = number_format($montoDisminucionTotal,2,',','.'); // Calculando el Total Disminución
	$montoPagadoTotal = number_format($montoPagadoTotal,2,',','.');      // Calculando el Total Pagado
	$montoAjustadoTotal = number_format($montoAjustadoTotal,2,',','.');      // Calculando el Total Ajustado
	$montoDisponibleTotal = number_format($montoDisponibleTotal,2,',','.');      // Calculando el Total Disponible
	                                             ///
	$montoT=number_format($montoT,2,',','.'); /// monto total partida
	
	$pdf->SetDrawColor(0, 0, 0); $pdf->SetFillColor(202, 202, 202); $pdf->SetTextColor(0, 0, 0);
	$pdf->SetFont('Arial', 'B', 8.5);
	 $pdf->SetWidths(array(18, 100,23,23,23,23,23,23));
	$pdf->SetAligns(array('C','R','R','R','R','R','R','R'));
	$pdf->Row(array('' ,'Total:',$montoTotal,$montoIncrementoTotal,$montoDisminucionTotal,$montoAjustadoTotal,$montoPagadoTotal,$montoDisponibleTotal));
	/////
//---------------------------------------------------*/
}/*
$pdf->SetDrawColor(0, 0, 0); $pdf->SetFillColor(202, 202, 202); $pdf->SetTextColor(0, 0, 0);
