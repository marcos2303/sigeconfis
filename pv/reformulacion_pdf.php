...
$SQLP="SELECT * 
		...
		WHERE CodRef='".$_GET['registro']."' AND
		      Organismo='".$_SESSION['ORGANISMO_ACTUAL']."'";
$QRYP=mysql_query($SQLP) or die ($SQLP.mysql_error());
$FPRE=mysql_fetch_array($QRYP);
	
	$qry=mysql_query("SELECT Sector,Programa,SubPrograma,Proyecto,Actividad,Organismo,CodPresupuesto,UnidadEjecutora 
	                    FROM pv_presupuesto 
					   WHERE CodPresupuesto='".$_GET['registro']."'") or die ($sql.mysql_error());
	...
	$pdf->Cell(140, 10, 'Descripción de Ajuste Presupuestario', 0, 1, 'C');	
	$pdf->Cell(72, 10, 'Ejercicio', 0, 0, 'R'); $pdf->Cell(40, 10, $FPRE['0'], 0, 1, 'L');	
	$pdf->SetFont('Arial', '', 7);
	...
	$pdf->Cell(25, 3.5, 'MTO. PRESU.', 1, 0, 'C', 1);
	$pdf->Cell(25, 3.5, 'MTO. AJUST.', 1, 1, 'C', 1);
...
//// --------------------------------------------------------------------------------------------------------------
//// --------------------------------------------------------------------------------------------------------------
 list($Organismo, $CodPresupuesto, $CodRef) = split('[-]', $_GET['registro']);
 $sql = "select 
                CodRef,
				CodPresupuesto,
				Descripcion,
 ...
 if($row!=0) $fiel = mysql_fetch_array($qry);
    
	$pdf->SetFont('Arial', 'B', 10);
	$pdf->Cell(160, 20, 'Descripción de Reformulación', 0, 1, 'C');	// Titulo
	
	$pdf->SetFont('Arial', 'B', 7);
	$pdf->Cell(40, 6, 'Nro. de Reformulación: ', 0, 0, 'R'); $pdf->Cell(10, 7, $fiel['CodRef'], 0, 0, 'L');
	$pdf->Cell(40, 6, 'Nro. de Presupuesto: ', 0, 0, 'R'); $pdf->Cell(10, 7, $fiel['CodPresupuesto'], 0, 0, 'L');
	$pdf->Cell(40, 6, 'Estado: ', 0, 0, 'R'); $pdf->Cell(10, 7, $fiel['CodPresupuesto'], 0, 1, 'L');
	
	$pdf->SetFont('Arial', 'B', 7);
	$pdf->Cell(40, 6, 'Descripcion: ', 0, 0, 'R');
	$pdf->Cell(10, 7, $fiel['Descripcion'], 0, 1, 'L');
	
	
	$sprep ="select * from mastpersonas where CodPersona='".$fiel['PreparadoPor']."'";
	$qprep = mysql_query($sprep) or die ($sprep.mysql_error()); 
	$fprep = mysql_fetch_array($qprep);
	
	$f_reformulacion = date("Y-m-d", strtotime($fiel['FechaRef']));
	$f_aprobacion = date("Y-m-d", strtotime($fiel['FechaAprobacion']));
	
	
	if($fiel['Estado']!='AP'){
	  $pdf->SetFont('Arial', 'B', 7);
	  $pdf->Cell(40, 6, 'Elaborado Por: ', 0, 0, 'R');$pdf->Cell(10, 7, $fprep['NomCompleto'], 0, 1, 'L');
	  $pdf->Cell(40, 6, 'Fecha Elaboración: ', 0, 0, 'R');$pdf->Cell(10, 7, $f_reformulacion, 0, 1, 'L');
	}else{
	  $pdf->SetFont('Arial', 'B', 7);
	  $pdf->Cell(40, 6, 'Elaborado Por: ', 0, 0, 'R');$pdf->Cell(10, 7, $fprep['NomCompleto'], 0, 1, 'L');
	  $pdf->Cell(40, 6, 'Fecha Elaboración: ', 0, 0, 'R');$pdf->Cell(10, 7, $f_reformulacion, 0, 0, 'L');
	  $pdf->Cell(40, 6, 'Fecha Aprobación: ', 0, 0, 'R');$pdf->Cell(10, 7, $f_aprobacion, 0, 1, 'L');
...
//	Cuerpo
/// CONSULTO PV_PRESUPUESTODET PARA VERIFICAR SI EL ANTEPROYECTO TIENE PARTIDAS ASIGNADAS ///
$sqlDet="SELECT cod_partida,MontoAprobado,
		  ...
		  WHERE CodPresupuesto='".$_GET['registro']."' 
...
for($i=0; $i<$rows ; $i++){
 ...
 if($fieldet['partida']!=00){
	 ...
	 $qry=mysql_query($sql) or die ($sql.mysql_error());
	 $field=mysql_fetch_array($qry);
	 $monto=$fieldet['MontoPresupuestado'] + $monto;
	 ...
	 $pdf->Row(array($fieldet['cod_partida'],$field['denominacion'],$montoT,''));
 }
}
	///// *** Mostrar *** /////
	...
	$pdf->SetWidths(array(25, 100, 25));
	$pdf->Cell(1,5);
	$pdf->SetAligns(array('C','R','L','L','C','L'));
	$pdf->Row(array('' ,'Total:',$montoT,''));
	/////
