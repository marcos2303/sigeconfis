...
<form name="frmentrada" id="frmentrada" action="gehen.php?anz=lg_almacen_despacho_lista" method="POST" onsubmit="return almacen_despacho(this);">
...
<table width="1000" class="tblForm">
    ...
    <tr>
		...
		<td>
			<select id="CodAlmacen" style="width:200px;">
            	<?=loadSelectAlmacen($field_requerimiento['CodAlmacen'], "N", 1)?>
			</select>
		</td>
	...
	<tr>
		...
		<td>
			<input type="hidden" id="ReferenciaNroDocumento" value="<?=$field_requerimiento['CodRequerimiento']?>" style="width:125px;" class="disabled" readonly />
			<input type="text" id="DocumentoReferenciaInterno" value="<?=$field_requerimiento['CodInterno']?>" style="width:125px;" class="disabled" readonly />
		</td>
        <td>&nbsp;</td>
    	<td>
...
<table width="100%" class="tblLista">
	...
	foreach ($detalle as $linea) {
		list($Requerimiento, $CodItem, $CodInterno, $Descripcion, $CodUnidad, $CantidadPedida, $CantidadPendiente, $StockActual, $CodCentroCosto, $EnTransito, $FlagCompraAlmacen) = split(";char:td;", $linea);
		...
		<tr class="trListaBody" onclick="mClk(this, 'sel_detalles');" id="detalles_<?=$nrodetalles?>">
			...
			<td align="center">
            	...
            	<input type="hidden" name="ReferenciaNroDocumento" value="<?=$field_requerimiento['CodRequerimiento']?>" />
            	<input type="hidden" name="ReferenciaNroInterno" value="<?=$field_requerimiento['CodInterno']?>" />
            	<input type="hidden" name="ReferenciaSecuencia" value="<?=$Secuencia?>" />
                RQ-<?=$field_requerimiento['CodInterno']?>-<?=$Secuencia?>
			</td>
		</tr>
		<?
	}
