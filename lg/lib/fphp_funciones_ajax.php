...
elseif($accion == "mostrarTabDistribucionRequerimiento") {
	...
	foreach ($detalle as $linea) {
		list($_CantidadPedida, $_CodCuenta, $_CodCuentaPub20, $_cod_partida) = split(";char:td;", $linea);
...
elseif($accion == "mostrarTabDistribucionOrden") {
	...
	$detalle = split(";char:tr;", $detalles);
	foreach ($detalle as $linea) {		
		list($_CantidadPedida, $_PrecioUnit, $_FlagExonerado, $_cod_partida, $_CodCuenta, $_CodCuentaPub20) = split(";char:td;", $linea);
	...
	foreach ($_PARTIDA as $cod_partida) {
		if ($_PARTIDA_MONTO[$cod_partida] > 0) {
			$Descripcion = getValorCampo("pv_partida", "cod_partida", "denominacion", $cod_partida);
			$CodCuenta = $_PARTIDA_CUENTA[$cod_partida];
			$CodCuentaPub20 = $_PARTIDA_CUENTA20[$cod_partida];
			list($MontoAjustado, $MontoComprometido, $MontoPendiente) = disponibilidadPartida($Anio, $CodOrganismo, $cod_partida, $CodPresupuesto);
...
elseif($accion == "transaccion_commodity_recepcion_detalles") {
	...
	$sql = "SELECT
			...
			FROM
				...
				LEFT JOIN lg_commoditymast cm ON (cs.CommodityMast = cm.CommodityMast)
			WHERE
				ocd.Anio = '".$Anio."' AND
				ocd.CodOrganismo = '".$CodOrganismo."' AND
				ocd.NroOrden = '".$NroOrden."' AND
